"""M43U signal/interference comparison; downstream M43R arithmetic copied unchanged.
...
def calibrate(*,mask_policy,window,grid,bank,table,basis,scans,store,shifts,minimum_shift_bins,progress=lambda m:None):
    ...
    for t in range(len(bank)):
        ...
        mask=build_mask(arrays.__getitem__,mask_policy)[:,grid.score_slice]
...
def execute(*,mask_policy,calibration_binding,window,grid,bank,table,basis,scans,store,calibration,threshold,receiver_factory,maximum_records=10000,progress=lambda message:None):
    ...
    """
    validate_binding(calibration_binding,mask_policy,calibration,threshold)
    ...
    for kind in ('on','off'):
        for t in range(len(bank)):
            ...
            mask_counts[f'{kind}:{t}']=int(mask.sum())
    ...
    result={'schema':'m43u-signal-interference-diagnostic-v1','purpose':'native-signal-interference-comparison',
        'mask_policy':mask_policy,'calibration_binding':calibration_binding,
        'input_inventory_sha256':input_root,'catalogue_sha256':table.template_bank_sha256,
        'factor_table_sha256':table.factor_table_sha256,'mask_sha256s':mask_hashes,'masked_cell_counts':mask_counts,
