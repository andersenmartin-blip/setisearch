"""M43 Cartesian-bank bridge and integrated diagnostic detector entry point.
...
def execute(*,window,grid,bank,table,basis,scans,store,shifts,minimum_shift_bins,
            reference_floor,receiver_factory,maximum_records=10000,progress=lambda message:None):
    """Run calibration handoff, both retention passes and every physical stage.
    ...
    A future science run requires a separately frozen calibration protocol.
    """
    ...
    for kind in ('on','off'):
        for t in range(len(bank)):
            ...
            mask=core.build_m37_two_pass_template_mask(arrays.__getitem__)[:,grid.score_slice]
            ...
            if kind=='on':
                for wi,w in enumerate(widths):
                    core.update_calibration(acc,arrays[w][:,grid.score_slice],template_index=t,width_index=wi,
                                            exclusion_mask=masks[(kind,t)])
            progress(f'{kind} template {t}: mask and diagnostic calibration handoff')
    acc.finalize()
    threshold=core.calibrated_threshold((acc,),expected_window_ids=(window,),reference_floor=reference_floor,
    ...
    result={'schema':'m43q-integrated-diagnostic-v1','purpose':'integration-qualification-only',
        ...
        'scientific_candidate_selection_authorized':False,'fresh_null_calibration':False,
