"""One predeclared neighborhood-support alternative to the M37 isolation mask."""
import hashlib
import numpy as np
from . import search_v0p6 as core

POLICIES = ('legacy', 'neighbor9')
...
def build_mask(vector_factory, policy):
    ...
    for width in core.M37_SPECTRAL_WIDTHS:
        ...
        # the same-width clipped +/-9 carrier neighborhood. Never wrap edges.
        support = core.dilate_q_mask(values >= 3., 9)
...
def bind_calibration(calibration, threshold, policy):
    ...
    record = {'schema': 'm43t-mask-calibration-binding-v1', 'mask_policy': policy,
