#!/usr/bin/env python3
"""Readable report and figure from retained LS8AW tables and audit, no network."""
import gzip
import hashlib
import json
...
OUT = ROOT/'results_ls8aw_l2_screen'


def main():
    s = json.loads((OUT/'summary.json').read_text())
    a = json.loads((OUT/'audit.json').read_text())
    meta = json.loads((ROOT/'results_ls8aw_l2_metadata/summary.json').read_text())
    ...
    for column, visit in enumerate(s['visits']):
        ...
        axes[0, column].set_title(visit['file_key'].split('_')[2]+' · EC14338-1445')
    ...
    fig.suptitle('LS8AW · two prospectively selected CHEOPS visits', fontsize=15, fontweight='bold')
    fig.savefig(OUT/'ec143381445_screen.png', dpi=180)
    plt.close(fig)
    rows = []
    for v in s['visits']:
    ...
    if representatives:
        ...
        followup='Close this two-visit EC14338-1445 screen as a descriptive null. The next independent cohort is rank 20 of the unchanged reconciled LS8J ledger, GJ 536. Freeze its exact pair and header preflight before accessing that cohort. Do not widen or retune the closed EC14338-1445 pair.'
        reps=''
    if a['status']!='PASS':
    ...
    text=f'''# LS8AW — EC14338-1445 two-visit DEFAULT-L2 screen
...
![Retained light curves and all eligible signed scores](ec143381445_screen.png)
...
EC14338-1445 was rank 19 of the metadata-only first-1,000-public-row CHEOPS census:
