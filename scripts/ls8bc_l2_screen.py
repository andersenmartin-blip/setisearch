#!/usr/bin/env python3
"""Transfer pinned LS8K acquisition/scoring functions to the frozen GJ 422 pair."""
import argparse
import hashlib
import json
...
KEYS = ['CH_PR100018_TG012201_V0300', 'CH_PR100018_TG012202_V0300']
...
def main():
    ...
    assert not OUT.exists(), 'refuse LS8BC output overwrite'
    cfg = json.loads((ROOT / 'config/ls8bc_l2_scope.json').read_text())
    ...
    summary = {'stage': 'LS8BC_GJ422_TWO_VISIT_L2_SCREEN', 'status': 'COMPLETE_UNAUDITED',
        'evaluation_freeze_commit': args.freeze, 'target': 'GJ 422', 'selected_keys': KEYS,
        ...
        'interpretation': 'Independent rank-22 two-visit L2 screen; signed endpoints are not Gaussian significances.'}
