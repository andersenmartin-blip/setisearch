#!/usr/bin/env python3
"""Checkpointed LS5 screen through the unchanged LS4B SIGPROC/LS1 detector."""
...
def run(config_path,out,data_dir):
    ...
    if config['artifact_type']!='seti_repeater.ls5_preregistration' or config['freeze_boundary']['medium_resolution_values_read_before_freeze']:raise RuntimeError('invalid freeze')
    ...
    verify_detector_inheritance(config,Path('config/ls1_hd219134_light_sail.json'))
    ...
    if preflight['spectral_values_read'] or preflight['selected_for_preregistration']['cadence_url']!=config['archive_inventory']['selected_cadence_url']:raise RuntimeError('preflight selection mismatch')
    ...
    atomic_write(Path('DATA_MANIFEST_LS5.sha256'),manifest.encode())
...
if __name__=='__main__':
    ...
    run(a.config,Path('results_ls5_screen'),Path('data_ls5'))
