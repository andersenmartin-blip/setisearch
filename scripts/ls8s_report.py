#!/usr/bin/env python3
"""Readable report and figure from retained LS8S tables and audit, no network."""
import gzip
import hashlib
import json
...
def main():
    ...
    meta = json.loads((ROOT/'results_ls8s_l2_metadata/summary.json').read_text())
    ...
    for column, visit in enumerate(s['visits']):
        ...
        axes[0, column].set_title(visit['file_key'].split('_')[2]+' · TESS_260647166')
    ...
    fig.suptitle('LS8S · two prospectively selected CHEOPS visits', fontsize=15, fontweight='bold')
    fig.savefig(OUT/'tess260647166_screen.png', dpi=180)
    plt.close(fig)
    rows = []
    for v in s['visits']:
    ...
    if representatives:
        ...
        followup='Close this two-visit TESS_260647166 screen as a descriptive null. The next independent cohort is rank 5 of the unchanged reconciled LS8J ledger, EC 12578-2107. Freeze its exact pair and header preflight before accessing that cohort. Do not widen or retune the closed TESS_260647166 pair.'
        reps=''
    if a['status']!='PASS':
    ...
    text=f'''# LS8S — TESS_260647166 two-visit DEFAULT-L2 screen
