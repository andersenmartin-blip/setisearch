#!/usr/bin/env python3
"""Report every fixed LS8AP image outcome without altering the diagnostics."""
import json
import math
...
def main():
    ...
    if unresolved:
        ...
        next_action=('Close this WASP-103 follow-up under its fixed descriptive label without widening. '
                     'The next independent target is rank 15 of the unchanged reconciled LS8J ledger, GJ 581; '
                     'freeze its exact pair and header preflight before science values.')
    count=audit['comparisons']
    text=f'''# LS8AP — WASP-103 complete positive-event paired-image follow-up
