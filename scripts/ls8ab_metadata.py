#!/usr/bin/env python3
"""Acquire only named CAL/COR headers and exposure metadata for LS8AB."""
...
OUT = ROOT / 'results_ls8ab_metadata'
SEARCH_KEYS = ['CH_PR100016_TG007801_V0300', 'CH_PR100016_TG007802_V0300']
KEYS = ['CH_PR100016_TG007801_V0300']
...
def main():
    ...
    cfg=json.loads((ROOT/'config/ls8ab_representatives.json').read_text())
    for name,digest in cfg['input_pins'].items():
        assert hashlib.sha256((ROOT/name).read_bytes()).hexdigest()==digest,name
    assert json.loads((ROOT/'results_ls8aa_l2_screen/audit.json').read_text())['status']=='PASS'
    for line in (ROOT/'results_ls8aa_l2_screen/SHA256SUMS').read_text().splitlines():
        digest,name=line.split('  ',1)
        assert hashlib.sha256((ROOT/'results_ls8aa_l2_screen'/name).read_bytes()).hexdigest()==digest,name
    expected=[]
    ...
    assert not OUT.exists(), 'refuse completed LS8AB metadata overwrite'
    OUT.mkdir()
