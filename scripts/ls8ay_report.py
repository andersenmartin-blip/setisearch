#!/usr/bin/env python3
"""Report every fixed LS8AY image outcome without altering the diagnostics."""
import json
import math
...
def main():
    ...
    cfg=json.loads((ROOT/'config/ls8ay_images.json').read_text())
    assert len(rows)==9 and [r['id'] for r in rows]==[c['id'] for c in cfg['contexts']]
    ...
    if unresolved:
        ...
        next_action=('Close this GJ 536 follow-up under its fixed descriptive label without widening. '
                     'The next independent target is rank 21 of the unchanged reconciled LS8J ledger, 2MASS J11285624+1010395; '
                     'freeze its exact pair and header preflight before science values.')
    l2meta=json.loads((ROOT/'results_ls8ax_l2_metadata/summary.json').read_text())
    cadence={p['file_key']:p['keywords']['TEXPTIME'] for p in l2meta['products']}
    details=[]
    for context in cfg['contexts']:
        key=context['file_key'];sign=context['sign'];cid=context['cluster_id']
        original=json.loads((ROOT/'results_ls8ax_l2_screen'/key/'clusters.json').read_text())
        matches=[c['representative'] for c in original[sign] if c['cluster_id']==cid]
        assert len(matches)==1
        rep=matches[0]
        assert (rep['start'],rep['duration'])==(context['start'],context['duration'])
        duration=cadence[key]*rep['duration']
        percent=100*rep['excess_electrons']/rep['baseline_event_sum_electrons']
        details.append(f"| {context['id']} | {rep['start']} | {duration:.8g} s | {rep['score']:+.12f} | {percent:+.6f}% |")
    original_event_table='\n'.join(details)
    count=audit['comparisons']
    text=f'''# LS8AY — GJ 536 complete signed-event paired-image follow-up
