#!/usr/bin/env python3
"""Acquire only named CAL/COR headers and exposure metadata for LS8AG."""
...
OUT = ROOT / 'results_ls8ag_metadata'
SEARCH_KEYS = ['CH_PR100002_TG000901_V0300', 'CH_PR100002_TG000902_V0300']
KEYS = ['CH_PR100002_TG000901_V0300']
...
def main():
    ...
    cfg=json.loads((ROOT/'config/ls8ag_representatives.json').read_text())
    for name,digest in cfg['input_pins'].items():
        assert hashlib.sha256((ROOT/name).read_bytes()).hexdigest()==digest,name
    assert json.loads((ROOT/'results_ls8af_l2_screen/audit.json').read_text())['status']=='PASS'
    for line in (ROOT/'results_ls8af_l2_screen/SHA256SUMS').read_text().splitlines():
        digest,name=line.split('  ',1)
        assert hashlib.sha256((ROOT/'results_ls8af_l2_screen'/name).read_bytes()).hexdigest()==digest,name
    expected=[]
    ...
    assert not OUT.exists(), 'refuse completed LS8AG metadata overwrite'
    OUT.mkdir()
