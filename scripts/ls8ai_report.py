#!/usr/bin/env python3
"""Readable report and figure from retained LS8AI tables and audit, no network."""
import gzip
import hashlib
import json
...
OUT = ROOT/'results_ls8ai_l2_screen'


def main():
    s = json.loads((OUT/'summary.json').read_text())
    a = json.loads((OUT/'audit.json').read_text())
    meta = json.loads((ROOT/'results_ls8ai_l2_metadata/summary.json').read_text())
    ...
    for column, visit in enumerate(s['visits']):
        ...
        axes[0, column].set_title(visit['file_key'].split('_')[2]+' · EC13080-1508')
    ...
    fig.suptitle('LS8AI · two prospectively selected CHEOPS visits', fontsize=15, fontweight='bold')
    fig.savefig(OUT/'ec130801508_screen.png', dpi=180)
    plt.close(fig)
    rows = []
    for v in s['visits']:
    ...
    if representatives:
        ...
        followup='Close this two-visit EC13080-1508 screen as a descriptive null. The next independent cohort is rank 12 of the unchanged reconciled LS8J ledger, PG 1343-102. Freeze its exact pair and header preflight before accessing that cohort. Do not widen or retune the closed EC13080-1508 pair.'
        reps=''
    if a['status']!='PASS':
    ...
    text=f'''# LS8AI — EC13080-1508 two-visit DEFAULT-L2 screen
