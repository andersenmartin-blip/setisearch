#!/usr/bin/env python3
"""Frozen LS8AM range acquisition and paired-image analysis, with offline reuse."""
import argparse
...
def main():
    ...
    scope=json.loads((ROOT/'config/ls8am_payload_scope.json').read_text())
    for name,digest in scope['input_pins'].items():
        assert sha((ROOT/name).read_bytes())==digest,name
    cfg=json.loads((ROOT/'config/ls8am_images.json').read_text())
    assert cfg['status']=='METADATA_JOINED_IMAGES_CLOSED' and cfg['image_pixels_read']==0
    assert cfg['join_rows']==174 and cfg['image_science_bytes']==55680000 and cfg['smearing_bytes']==139200
    assert [(c['id'],c['start'],c['duration'],c['lo'],c['hi']) for c in cfg['contexts']]==[('TG000801_P0', 223, 1, 209, 238), ('TG000801_P1', 363, 1, 349, 378), ('TG001401_P0', 178, 1, 164, 193)]
    assert json.loads((ROOT/'results_ls8am_metadata/audit.json').read_text())['status']=='PASS'
    for line in (ROOT/'results_ls8am_metadata/SHA256SUMS').read_text().splitlines():
        digest,relative=line.split('  ',1)
        assert sha((ROOT/'results_ls8am_metadata'/relative).read_bytes())==digest,relative
    ...
    for c in cfg['contexts']:
        ...
        if key not in tables:
            header=fits.Header.fromstring((ROOT/'results_ls8al_l2_metadata'/key/'lightcurve_header.bin').read_bytes().decode(),sep='')
            tables[key]=np.frombuffer((ROOT/'results_ls8al_l2_screen'/key/'lightcurve_table.bin').read_bytes(),dtype=dtype(header))
