#!/usr/bin/env python3
"""Report every frozen LS8P outcome after independent verification."""
import json
...
OUT = ROOT / 'results_ls8p_residuals'
...
def main():
    ...
    lines = ['# LS8P: GJ 1132 three-exposure residual, noise and boundary study', '',
        'All five retained LS8O contexts, both products and both coordinate conventions are included. '
        'No new archive bytes or exposures were acquired. The five original LS8O labels remain '
        '**UNRESOLVED_WITHIN_FIXED_SCOPE**. LS8N had no positive threshold crossing.', '',
        '**Retrospective diagnostics only: energy ratios, covariance models and eight sideband '
        ...
        f'{audit["comparisons"]["exact"]:,} exact checks; no disagreements. Seventeen pre-analysis '
        'tests pass (nine new three-sum/boundary tests and eight tests of the inherited spatial algebra). '
        'All 120 signed known-template controls, 200 signed compact controls and 320 additive checks pass.', '',
        '## Fixed covariance and residual comparison', '',
        'Each native event is the sum of three rows. The IID reference uses the exact OLS prediction '
        'weights, including the 9/n baseline-estimation term. The second model estimates a pooled '
        'lag-1/lag-2 correlation from training-only pixel residuals with a Bartlett taper; all '
        'covariance between event and baseline rows enters the same quadratic form. Cadence gaps '
        'split the covariance into blocks. This small, estimated, separable covariance model is '
        'descriptive; longer correlations, nonstationarity and template-estimation uncertainty are '
        'not calibrated. Both models remain visible.', '',
        'The eight nonoverlapping three-row control targets each exclude a two-row guard from '
        'training. Their templates, variances and correlations are refitted without held values. '
        'They share training data and differ in leverage and position from the selected native '
        'event; the counts below are not p-values.', '',
        ...
        for conv, info in event['conventions'].items():
            for kind in ('CAL', 'COR'):
                ...
                lines.append(f'| {event["id"]} | {kind} / {conv} | {m["weighted_explained"]:.4%} | '
                    ...
                    f'{d["held_blocks_at_least_event"]}/8 | {d["temporal"]["correlated_to_iid_variance_factor"]:.6f} | {top:.4%} |')
    ...
    lines += ['', 'Pure brightness additions are 0.1% per exposure in each of three exposures. '
        'Their summed coefficient is recovered as ±0.003. Displacement additions are ±0.05 '
        'gradient units per exposure, not a nonlinear resampling of a star. Five fixed compact '
        'positions receive both signs with total amplitude five modeled event-pixel SDs, split '
        'equally across three rows. The table shows losses from a hypothetical displacement plus '
        'constant subtraction; no subtraction, event veto or new mask is adopted.', '',
    ...
    lines += ['', '## All COR residual maps and held-block controls', '',
        ...
        'pixels are divided by the modeled three-row SD, not Gaussian sigmas. Only the plotting '
    ...
    for event in data:
        ...
        for row, (conv, info) in enumerate(event['conventions'].items()):
            with np.load(OUT / event['id'] / f'{conv}_arrays.npz') as arr:
                ...
                for col, (values, label) in enumerate(zip(fields, ['Residual (ADU)', 'Residual / modeled three-row SD'])):
            ...
            ax.scatter(rows, vals, color='#356a83', s=28, label='Held three-row block')
            ...
            ax.set_title(f'{conv} · {d["held_blocks_at_least_event"]}/8 blocks >= native')
            ax.grid(alpha=.15); ax.legend(fontsize=8)
        fig.suptitle(f'{event["id"]} · fixed three-row residual study\n{event["original_ls8o_classification"]}', fontsize=13)
    ...
    lines += ['## Stop and interpretation boundaries', '',
        ...
        'a low ratio does not prove an ordinary-noise origin. Spatial and boundary structure '
        'describe what the fixed templates miss. All five LS8O labels and LS8N thresholds remain '
        'unchanged, with no positive LS8N crossing and no qualified candidate, detector or coverage.', '',
        'This single bounded study is closed regardless of result. Rank-3 HD 136352 remains next '
        'for a separately frozen independent transfer from the unchanged target ledger. Its science '
        'values remain unopened. No further GJ 1132 tuning is automatically authorized by this study. '
        'Raw imagettes, reserved TESS/M43 material and the unsent calibration request remain unchanged.', '',
        '[Protocol](../LS8P_THREE_SUM_PROTOCOL.md) · [Full diagnostics](diagnostics.json) · '
        '[Signed injections](injection_controls.json) · [Audit](audit.json) · '
        '[Original LS8O report](../results_ls8o_images/REPORT.md)', '']
    ...
    (OUT / 'next_action.json').write_text(json.dumps({'status': 'BOUNDED_STUDY_CLOSED',
        ...
        'next': 'Interpret complete LS8P; then separately freeze rank-3 HD 136352 chronological-pair metadata and L2 transfer.',
        'original_ls8o_labels_preserved': True}, indent=2) + '\n')
