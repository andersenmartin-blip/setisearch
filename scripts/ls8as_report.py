#!/usr/bin/env python3
"""Report every frozen LS8AS outcome after independent verification."""
import json
...
OUT = ROOT / 'results_ls8as_residuals'
...
def main():
    ...
    lines = ['# LS8AS: GJ 581 residual and duration-matched noise study', '',
        'Both unresolved LS8AR positive contexts, both products and both conventions are included. The other 12 representatives retain their closed image labels without additional native fitting. '
        'No new archive bytes or exposures were acquired. TG023701_P3 and TG023701_P5 remain '
        '**UNRESOLVED_WITHIN_FIXED_SCOPE** under the original LS8AR gate.', '',
        ...
        'TG023701_P3 and TG023701_P5 are each one 60-second exposure. The IID reference uses the exact OLS prediction weights, '
    ...
    lines += ['## Stop and interpretation boundaries', '',
        ...
        'The LS8AR classifications and LS8AQ thresholds remain unchanged. No technical-origin '
        'interpretation, qualified candidate, detector or coverage is established.', '',
        'This single bounded study is closed regardless of result. Rank-16 EC14599-2047 is next '
        'for a separately frozen independent transfer from the unchanged target ledger, exact '
        'pair CH_PR100002_TG010301_V0300 and CH_PR100002_TG010302_V0300. Its science values '
        'remain unopened. Further GJ 581 tuning is not part of this study. Raw imagettes, '
        'reserved TESS/M43 material and the unsent calibration request remain unchanged.', '',
        '[Protocol](../LS8AS_RESIDUAL_NOISE_PROTOCOL.md) · [Full diagnostics](diagnostics.json) · '
        '[Signed injections](injection_controls.json) · [Audit](audit.json) · '
        '[Original LS8AR report](../results_ls8ar_images/REPORT.md)', '']
    ...
    (OUT / 'next_action.json').write_text(json.dumps({'status': 'BOUNDED_STUDY_CLOSED',
        ...
        'next': 'Interpret complete LS8AS; then separately freeze rank-16 EC14599-2047 chronological-pair metadata and L2 transfer.',
        'original_ls8ar_labels_preserved': True}, indent=2) + '\n')
