#!/usr/bin/env python3
"""LS8P offline use of the five already published, pinned LS8O contexts."""
import gzip
import hashlib
import json
import os
...
from seti_repeater.cheops_three_sum_noise import (
    SIDE, EVENT, BLOCKS, training_rows, prepare, describe, boundary_account,
...
def main():
    ...
    assert cfg['side_rows'] == SIDE.tolist() and cfg['event_rows'] == EVENT.tolist()
    assert cfg['held_blocks'] == [b.tolist() for b in BLOCKS]
    ...
    for context in original['contexts']:
        ...
        header = fits.Header.fromstring((ROOT / 'results_ls8n_l2_metadata' / key / 'lightcurve_header.bin').read_bytes().decode(), sep='')
        rows = np.frombuffer((ROOT / 'results_ls8n_l2_screen' / key / 'lightcurve_table.bin').read_bytes(), dtype=dtype(header))[context['lo']:context['hi']]
        assert len(rows) == 31 and context['duration'] == 3
        ...
        for kind in ('CAL', 'COR'):
            ...
            assert len(raw) == context['ranges'][f'SCI_{kind}_SubArray']['count'] == 9920000
            cubes[kind] = np.frombuffer(raw, dtype='>f8').astype(float).reshape(31, 200, 200)
        ...
        item = {'id': context['id'], 'sign': context['sign'],
            'original_ls8o_classification': previous[context['id']]['classification'], 'conventions': {}}
        masks = []
        ...
        for number in (0, 1):
            ...
            for kind in ('CAL', 'COR'):
                state = prepare(times, cubes[kind], SIDE, EVENT, common, cc)
                ...
                for block in BLOCKS:
                    training = training_rows(block)
                    pseudo = prepare(times, cubes[kind], training, block, common, cc)
                ...
                detail['original_ls8o_unweighted_fits'] = previous[context['id']]['conventions'][conv]['fits'][kind]
                info[kind] = detail
        ...
        print(context['id'], '4 native fits and 32 held three-row product/convention cases complete', flush=True)
    ...
    save(OUT / 'summary.json', {'status': 'COMPLETE_UNAUDITED', 'contexts': len(summaries),
        'actual_product_conventions': 20, 'held_three_row_cases': 160,
        ...
        'qualified_coverage_added': False, 'original_ls8o_classifications_unchanged': True})
