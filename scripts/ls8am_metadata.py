#!/usr/bin/env python3
"""Acquire only named CAL/COR headers and exposure metadata for LS8AM."""
...
OUT = ROOT / 'results_ls8am_metadata'
SEARCH_KEYS = ['CH_PR100041_TG000801_V0300', 'CH_PR100041_TG001401_V0300']
KEYS = ['CH_PR100041_TG000801_V0300', 'CH_PR100041_TG001401_V0300']
...
def main():
    ...
    cfg=json.loads((ROOT/'config/ls8am_representatives.json').read_text())
    for name,digest in cfg['input_pins'].items():
        assert hashlib.sha256((ROOT/name).read_bytes()).hexdigest()==digest,name
    assert json.loads((ROOT/'results_ls8al_l2_screen/audit.json').read_text())['status']=='PASS'
    for line in (ROOT/'results_ls8al_l2_screen/SHA256SUMS').read_text().splitlines():
        digest,name=line.split('  ',1)
        assert hashlib.sha256((ROOT/'results_ls8al_l2_screen'/name).read_bytes()).hexdigest()==digest,name
    assert cfg['search_keys']==SEARCH_KEYS and cfg['metadata_keys']==KEYS
    expected=[]
    for key in SEARCH_KEYS:
        clusters=json.loads((ROOT/'results_ls8al_l2_screen'/key/'clusters.json').read_text())
        for sign in ('positive','negative'):
            for c in clusters[sign]:
                rep=c['representative'];expected.append([key,sign,c['cluster_id'],rep['start'],rep['duration']])
    assert cfg['representatives']==expected and len(expected)==3
    assert sum(28+r[4] for r in expected)==cfg['expected_context_rows']==87
    assert cfg['expected_cal_cor_joins']==174
    assert not OUT.exists(), 'refuse completed LS8AM metadata overwrite'
    OUT.mkdir()
