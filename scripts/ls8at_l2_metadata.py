#!/usr/bin/env python3
"""Bounded header-only preflight for the two frozen LS8AT EC145992047 visits."""
...
OUT=ROOT/'results_ls8at_l2_metadata'
BASE='https://cheops-webapp-pg.obsuksprd2.unige.ch/'
KEYS=['CH_PR100002_TG010301_V0300','CH_PR100002_TG010302_V0300']
...
def one(key):
 ...
 exp={
  'CH_PR100002_TG010301_V0300':(1,60.,60.),
  'CH_PR100002_TG010302_V0300':(1,60.,60.),
 }[key]
 assert int(kw['NEXP'])==exp[0]
 assert abs(float(kw['EXPTIME'])-exp[1])<1e-6 and abs(float(kw['TEXPTIME'])-exp[2])<1e-6
...
def main():
 ...
 cfg=json.loads((ROOT/'config/ls8at_selected_pair.json').read_text())
 ...
 assert recon['eligible_cohorts'][15]==cfg['cohort']
 assert cfg['cohort']['rank']==16 and cfg['cohort']['normalized_target']=='ec145992047'
 ...
 s={'stage':'LS8AT_EC145992047_L2_HEADER_PREFLIGHT','status':'PASS_COMPATIBLE' if compatible else 'INCOMPATIBLE',
