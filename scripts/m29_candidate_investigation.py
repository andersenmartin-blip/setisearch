#!/usr/bin/env python3
"""Fixed post-hoc morphology checks for the two Milestone 29 review cases.

The implementation reuses the published Milestone 11 local-cutout
measurements without modifying that historical script. It evaluates the two
Milestone 29 arithmetic-family cases together so the fixed cross-candidate
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("config/hd11964b_heldout_m29.json"),
    )
    parser.add_argument(
        "--summary",
        type=Path,
        default=Path("results_m29/search_summary.json"),
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
        default=Path("data_m29_candidate_investigation"),
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results_m29_candidate_investigation"),
    )
    parser.add_argument("--extract", action="store_true")
    parser.add_argument("--workers", type=int, default=12)
    parser.add_argument("--date-utc", default="2026-08-24")
    ...
    archive = {
        ...
        "interpretation": (
            "The Milestone 29 header-only screen found no second qualifying "
            "HD 11964 cadence. Any unresolved case therefore requires a new "
            "independent observation rather than another public archive cadence."
        ),
    }
    result = {
