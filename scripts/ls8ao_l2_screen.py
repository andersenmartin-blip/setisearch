#!/usr/bin/env python3
"""Transfer pinned LS8K acquisition/scoring functions to the frozen WASP-103 pair."""
import argparse
import hashlib
import json
...
META = ROOT / 'results_ls8ao_l2_metadata'
...
def main():
    ...
    assert not OUT.exists(), 'refuse LS8AO output overwrite'
    cfg = json.loads((ROOT / 'config/ls8ao_l2_scope.json').read_text())
    ...
    summary = {'stage': 'LS8AO_WASP103_TWO_VISIT_L2_SCREEN', 'status': 'COMPLETE_UNAUDITED',
        'evaluation_freeze_commit': args.freeze, 'target': 'WASP-103', 'selected_keys': KEYS,
        ...
        'interpretation': 'Independent rank-14 two-visit L2 screen; signed endpoints are not Gaussian significances.'}
