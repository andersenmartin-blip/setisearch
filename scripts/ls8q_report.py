#!/usr/bin/env python3
"""Readable report and figure from retained LS8Q tables and audit, no network."""
import gzip
import hashlib
import json
...
OUT = ROOT/'results_ls8q_l2_screen'


def main():
    s = json.loads((OUT/'summary.json').read_text())
    a = json.loads((OUT/'audit.json').read_text())
    meta = json.loads((ROOT/'results_ls8q_l2_metadata/summary.json').read_text())
    ...
    for column, visit in enumerate(s['visits']):
        ...
        axes[0, column].set_title(visit['file_key'].split('_')[2]+' · HD 136352')
    ...
    fig.suptitle('LS8Q · two prospectively selected CHEOPS visits', fontsize=15, fontweight='bold')
    fig.savefig(OUT/'hd136352_screen.png', dpi=180)
    plt.close(fig)
    rows = []
    for v in s['visits']:
    ...
    if representatives:
        ...
        followup='Close this two-visit HD 136352 screen as a descriptive null. The next independent cohort is rank 4 of the unchanged reconciled LS8J ledger, TESS_260647166. Freeze its exact pair and header preflight before accessing that cohort. Do not widen or retune the closed HD 136352 pair.'
        reps=''
    if a['status']!='PASS':
    ...
    text=f'''# LS8Q — HD 136352 two-visit DEFAULT-L2 screen

Completed 20 September 2026. Result: **{s['status']}**.
...
`{s['evaluation_freeze_commit']}`. Both products have TEXPTIME approximately
