"""Frozen M43X three-endpoint confirmation trial on shared base executions."""
import argparse
import gzip
import hashlib
import json
...
from seti_repeater.confirmation_m43x import POLICIES,remaining_evidence,apply_controls

ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'results_m43x_confirmation'
CONFIG=ROOT/'config/m43x_confirmation.json';WINDOW='m43x-shared-pre-veto-confirmation'

def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()

def validate_freeze(freeze):
    cfg=json.loads(CONFIG.read_text())
    assert subprocess.check_output(['git','show',freeze+':config/m43x_confirmation.json'],cwd=ROOT)==CONFIG.read_bytes()
...
def run(anchor_root,source_root,checkpoint_root,freeze,preflight_only=False):
    ...
    for t in range(len(bank)):
        for w in core.M37_SPECTRAL_WIDTHS:
            values,_=baseline.get('on',t,w)
            for q in cfg['anchor_query_positions']:
                on=values[:,grid.score_slice.start+q].tolist()
                for active in core.M37_ACTIVITY_SUBSETS:
                    actual=remaining_evidence(on,active)
                    ordered=sorted(on[e] for e in active)
                    reference=math.fsum(ordered[:-1])/math.sqrt(len(active)-1)
                    assert actual['remaining_score']==reference
                    anchors.append({'template':t,'width':w,'q':q,'active':list(active),'evidence':actual})
    assert len(anchors)==5920
    write_sealed(OUT/('preflight_anchors.json' if preflight_only else 'anchors.json'),{'freeze_commit':freeze,'all_96_arrays_exact':True,'all_48_native_gathers_exact':True,
        'remaining_scalar_comparisons':5920,'queries':anchors})
    print('All original arrays/gathers and 5,920 aggregate scalar anchors exact',flush=True)
    if preflight_only:
        print('M43X PREFLIGHT COMPLETE; no calibration or prospective input evaluated',flush=True)
        return
    ...
    for case in cfg['cases']:
        ...
        if path.exists():
            ...
            rec=write_sealed(path,{'freeze_commit':freeze,'config_sha256':sha(CONFIG),'case':case,'binding':binding,
                ...
                'policy_decisions':{p:[{k:m[k] for k in ('record_id','passes_evaluated_physical_vetoes','physical_disposition','m43x_rejections')} for m in a['members']] for p,a in audits.items()},
                'endpoints':rows})
        paths.append(path);endpoints.extend(rec['endpoints'])
        print(f'{len(paths)}/256 {case["case_type"]} S={case["strength"]}: '+str({e['policy']:(e['truth_association']['recovered'],e['final_members']) for e in rec['endpoints']}),flush=True)
        write_sealed(OUT/'progress.json',{'complete':False,'completed_inputs':len(paths),'policy_endpoints':len(endpoints)})
    assert len(paths)==256 and len(endpoints)==768
    ...
    write_sealed(OUT/'result.json',{'milestone':'M43X','complete':True,'freeze_commit':freeze,'inputs':256,
        'base_detector_executions':256,'policy_endpoints':768,'separate_baseline_executions':1,
    ...
    write_sealed(OUT/'progress.json',{'complete':True,'completed_inputs':256,'policy_endpoints':768})
    print('M43X COMPLETE',flush=True)
