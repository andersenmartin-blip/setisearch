#!/usr/bin/env python3
"""Frozen LS8O range acquisition and paired-image analysis, with offline reuse."""
import argparse
...
def main():
    ...
    cfg=json.loads((ROOT/'config/ls8o_images.json').read_text())
    assert cfg['status']=='METADATA_JOINED_IMAGES_CLOSED' and cfg['image_pixels_read']==0
    assert cfg['join_rows']==310 and cfg['image_science_bytes']==99200000 and cfg['smearing_bytes']==248000
    assert [(c['id'],c['start'],c['duration'],c['lo'],c['hi']) for c in cfg['contexts']]==[
        ('TG000401_N0',88,3,74,105),('TG000403_N0',69,3,55,86),('TG000403_N1',132,3,118,149),
        ('TG000403_N2',191,3,177,208),('TG000403_N3',297,3,283,314)]
    assert json.loads((ROOT/'results_ls8o_metadata/audit.json').read_text())['status']=='PASS'
    for line in (ROOT/'results_ls8o_metadata/SHA256SUMS').read_text().splitlines():
        digest,relative=line.split('  ',1)
        assert sha((ROOT/'results_ls8o_metadata'/relative).read_bytes())==digest,relative
    ...
    for c in cfg['contexts']:
        ...
        if key not in tables:
            ...
            tables[key]=np.frombuffer((ROOT/'results_ls8n_l2_screen'/key/'lightcurve_table.bin').read_bytes(),dtype=dtype(header))
