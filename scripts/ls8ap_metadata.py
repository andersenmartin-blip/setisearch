#!/usr/bin/env python3
"""Acquire only named CAL/COR headers and exposure metadata for LS8AP."""
...
OUT = ROOT / 'results_ls8ap_metadata'
SEARCH_KEYS = ['CH_PR100013_TG000101_V0300', 'CH_PR100013_TG000102_V0300']
KEYS = ['CH_PR100013_TG000102_V0300']
...
def main():
    ...
    cfg=json.loads((ROOT/'config/ls8ap_representatives.json').read_text())
    for name,digest in cfg['input_pins'].items():
        assert hashlib.sha256((ROOT/name).read_bytes()).hexdigest()==digest,name
    assert json.loads((ROOT/'results_ls8ao_l2_screen/audit.json').read_text())['status']=='PASS'
    for line in (ROOT/'results_ls8ao_l2_screen/SHA256SUMS').read_text().splitlines():
        digest,name=line.split('  ',1)
        assert hashlib.sha256((ROOT/'results_ls8ao_l2_screen'/name).read_bytes()).hexdigest()==digest,name
    assert cfg['search_keys']==SEARCH_KEYS and cfg['metadata_keys']==KEYS
    expected=[]
    for key in SEARCH_KEYS:
        clusters=json.loads((ROOT/'results_ls8ao_l2_screen'/key/'clusters.json').read_text())
    ...
    assert not OUT.exists(), 'refuse completed LS8AP metadata overwrite'
    OUT.mkdir()
