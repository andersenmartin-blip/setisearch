#!/usr/bin/env python3
"""Transfer pinned LS8K acquisition/scoring functions to the frozen GJ 536 pair."""
import argparse
import hashlib
import json
...
def main():
    ...
    assert not OUT.exists(), 'refuse LS8AX output overwrite'
    cfg = json.loads((ROOT / 'config/ls8ax_l2_scope.json').read_text())
    ...
    summary = {'stage': 'LS8AX_GJ536_TWO_VISIT_L2_SCREEN', 'status': 'COMPLETE_UNAUDITED',
        'evaluation_freeze_commit': args.freeze, 'target': 'GJ 536', 'selected_keys': KEYS,
        ...
        'interpretation': 'Independent rank-20 two-visit L2 screen; signed endpoints are not Gaussian significances.'}
