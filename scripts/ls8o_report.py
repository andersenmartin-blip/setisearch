#!/usr/bin/env python3
"""Report every fixed LS8O image outcome without altering the diagnostics."""
import json
import math
...
def main():
    ...
    assert len(rows)==5 and [r['id'] for r in rows]==[c['id'] for c in cfg['contexts']]
    ...
    if unresolved:
        ...
        next_action=('Close all five GJ 1132 follow-up branches under their fixed descriptive labels without widening. '
                     'The next independent target is rank 3 of the unchanged reconciled LS8J ledger, HD 136352; '
                     'freeze its exact pair and header preflight before science values.')
    count=audit['comparisons']
    text=f'''# LS8O — all five GJ 1132 paired-image follow-ups
...
LS8N had no positive threshold crossing; none is added or substituted here. Its outcomes are: **{json.dumps(summary['outcomes'],sort_keys=True)}**.
