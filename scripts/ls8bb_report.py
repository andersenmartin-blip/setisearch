#!/usr/bin/env python3
"""Report every fixed LS8BB image outcome without altering the diagnostics."""
import json
import math
...
def main():
    ...
    assert len(rows)==2 and [r['id'] for r in rows]==[c['id'] for c in cfg['contexts']]
    ...
    if unresolved:
        ...
        next_action=('Close this 2MASS J11285624+1010395 follow-up under its fixed descriptive label without widening. '
                     'The next independent target is rank 22 of the unchanged reconciled LS8J ledger, GJ 422; '
                     'freeze its exact pair and header preflight before science values.')
    l2meta=json.loads((ROOT/'results_ls8ba_l2_metadata/summary.json').read_text())
    ...
    for context in cfg['contexts']:
        ...
        original=json.loads((ROOT/'results_ls8ba_l2_screen'/key/'clusters.json').read_text())
    ...
    text=f'''# LS8BB — 2MASS J11285624+1010395 complete signed-event paired-image follow-up
