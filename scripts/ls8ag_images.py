#!/usr/bin/env python3
"""Frozen LS8AG range acquisition and paired-image analysis, with offline reuse."""
import argparse
...
def main():
    ...
    scope=json.loads((ROOT/'config/ls8ag_payload_scope.json').read_text())
    ...
    assert [(c['id'],c['start'],c['duration'],c['lo'],c['hi']) for c in cfg['contexts']]==[
        ('TG000901_P0',60,1,46,75)]
    assert json.loads((ROOT/'results_ls8ag_metadata/audit.json').read_text())['status']=='PASS'
    for line in (ROOT/'results_ls8ag_metadata/SHA256SUMS').read_text().splitlines():
        digest,relative=line.split('  ',1)
        assert sha((ROOT/'results_ls8ag_metadata'/relative).read_bytes())==digest,relative
    ...
    for c in cfg['contexts']:
        ...
        if key not in tables:
            ...
            tables[key]=np.frombuffer((ROOT/'results_ls8af_l2_screen'/key/'lightcurve_table.bin').read_bytes(),dtype=dtype(header))
