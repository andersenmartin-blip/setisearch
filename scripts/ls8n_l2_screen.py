#!/usr/bin/env python3
"""Transfer pinned LS8K acquisition/scoring functions to the frozen GJ 1132 pair."""
import argparse
import hashlib
import json
...
META = ROOT / 'results_ls8n_l2_metadata'
...
def main():
    ...
    assert not OUT.exists(), 'refuse LS8N output overwrite'
    cfg = json.loads((ROOT / 'config/ls8n_l2_scope.json').read_text())
    ...
    summary = {'stage': 'LS8N_GJ1132_TWO_VISIT_L2_SCREEN', 'status': 'COMPLETE_UNAUDITED',
        'evaluation_freeze_commit': args.freeze, 'target': 'GJ 1132', 'selected_keys': KEYS,
        ...
        'interpretation': 'Independent rank-2 two-visit L2 screen; signed endpoints are not Gaussian significances.'}
