#!/usr/bin/env python3
"""Readable report and figure from retained LS8Y tables and audit, no network."""
import gzip
import hashlib
import json
...
OUT = ROOT/'results_ls8y_l2_screen'


def main():
    s = json.loads((OUT/'summary.json').read_text())
    a = json.loads((OUT/'audit.json').read_text())
    meta = json.loads((ROOT/'results_ls8y_l2_metadata/summary.json').read_text())
    ...
    for column, visit in enumerate(s['visits']):
        ...
        axes[0, column].set_title(visit['file_key'].split('_')[2]+' · PG 1245-042')
    ...
    fig.suptitle('LS8Y · two prospectively selected CHEOPS visits', fontsize=15, fontweight='bold')
    fig.savefig(OUT/'pg1245042_screen.png', dpi=180)
    plt.close(fig)
    rows = []
    for v in s['visits']:
    ...
    if representatives:
        ...
        followup='Close this two-visit PG 1245-042 screen as a descriptive null. The next independent cohort is rank 8 of the unchanged reconciled LS8J ledger, WASP-43. Freeze its exact pair and header preflight before accessing that cohort. Do not widen or retune the closed PG 1245-042 pair.'
        reps=''
    if a['status']!='PASS':
    ...
    text=f'''# LS8Y — PG 1245-042 two-visit DEFAULT-L2 screen
...
![Retained light curves and all eligible signed scores](pg1245042_screen.png)
...
PG 1245-042 was rank 7 of the metadata-only first-1,000-public-row CHEOPS census:
...
Image bytes are zero; no alternative aperture, raw imagette or later PG 1245-042
visit was opened.
