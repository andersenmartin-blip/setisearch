#!/usr/bin/env python3
"""Independent struct/long-double/normal-equation audit of LS8H."""
import argparse
import gzip
import hashlib
import json
...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'results_ls8h_images'
LD=np.longdouble
...
def metadata_audit(cfg):
    ...
    for key,sources in cfg['sources'].items():
        l2[key]=list(L2ROW.iter_unpack((ROOT/'results_ls8g_l2_screen'/key/'lightcurve_table.bin').read_bytes()))
    ...
    ledger=json.loads((ROOT/'results_ls8h_metadata/joins.json').read_text())
...
def main():
    ...
    cfg=json.loads((ROOT/'config/ls8h_images.json').read_text());l2,meta=metadata_audit(cfg)
    if args.metadata_only:
        (ROOT/'results_ls8h_metadata/audit.json').write_text(json.dumps(meta,indent=2)+'\n');print(json.dumps(meta));return
    ...
    audit={'status':'FAIL' if failures else 'PASS','comparisons':count,'maximum_absolute_differences':maxima,'disagreements':failures,
           ...
           'tolerances':{'relative':2e-8,'native_absolute':1e-6,'dimensionless_absolute':1e-8},'original_ls8b_audit':'FAIL'}
