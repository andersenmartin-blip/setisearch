#!/usr/bin/env python3
"""Independent struct/long-double/normal-equation audit of LS8D."""
import argparse
import gzip
import hashlib
import json
...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'results_ls8d_images'
LD=np.longdouble
...
def metadata_audit(cfg):
    ...
    for key,sources in cfg['sources'].items():
        l2[key]=list(L2ROW.iter_unpack((ROOT/'results_ls8b_l2_suite'/key/'lightcurve_table.bin').read_bytes()))
    ...
    ledger=json.loads((ROOT/'results_ls8d_metadata/joins.json').read_text())
...
def main():
    ...
    cfg=json.loads((ROOT/'config/ls8d_images.json').read_text());l2,meta=metadata_audit(cfg)
    if args.metadata_only:
        (ROOT/'results_ls8d_metadata/audit.json').write_text(json.dumps(meta,indent=2)+'\n');print(json.dumps(meta));return
