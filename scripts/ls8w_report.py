#!/usr/bin/env python3
"""Readable report and figure from retained LS8W tables and audit, no network."""
import gzip
import hashlib
import json
...
def main():
    ...
    meta = json.loads((ROOT/'results_ls8w_l2_metadata/summary.json').read_text())
    ...
    for column, visit in enumerate(s['visits']):
        ...
        axes[0, column].set_title(visit['file_key'].split('_')[2]+' · GJ 436')
    ...
    fig.suptitle('LS8W · two prospectively selected CHEOPS visits', fontsize=15, fontweight='bold')
    fig.savefig(OUT/'gj436_screen.png', dpi=180)
    plt.close(fig)
    rows = []
    for v in s['visits']:
    ...
    if representatives:
        ...
        followup='Close this two-visit GJ 436 screen as a descriptive null. The next independent cohort is rank 7 of the unchanged reconciled LS8J ledger, PG 1245-042. Freeze its exact pair and header preflight before accessing that cohort. Do not widen or retune the closed GJ 436 pair.'
        reps=''
    if a['status']!='PASS':
    ...
    text=f'''# LS8W — GJ 436 two-visit DEFAULT-L2 screen
...
![Retained light curves and all eligible signed scores](gj436_screen.png)
...
GJ 436 was rank 6 of the metadata-only first-1,000-public-row CHEOPS census:
...
Image bytes are zero; no alternative aperture, raw imagette or later GJ 436
visit was opened.
