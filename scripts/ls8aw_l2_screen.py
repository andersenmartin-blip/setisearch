#!/usr/bin/env python3
"""Transfer pinned LS8K acquisition/scoring functions to the frozen EC14338-1445 pair."""
import argparse
import hashlib
import json
...
META = ROOT / 'results_ls8aw_l2_metadata'
...
def main():
    ...
    assert not OUT.exists(), 'refuse LS8AW output overwrite'
    cfg = json.loads((ROOT / 'config/ls8aw_l2_scope.json').read_text())
    ...
    summary = {'stage': 'LS8AW_EC143381445_TWO_VISIT_L2_SCREEN', 'status': 'COMPLETE_UNAUDITED',
        'evaluation_freeze_commit': args.freeze, 'target': 'EC14338-1445', 'selected_keys': KEYS,
        ...
        'interpretation': 'Independent rank-19 two-visit L2 screen; signed endpoints are not Gaussian significances.'}
