#!/usr/bin/env python3
"""Report every frozen LS8AN outcome after independent verification."""
import json
...
OUT = ROOT / 'results_ls8an_residuals'
...
def main():
    ...
    lines = ['# LS8AN: HD 106315 residual and duration-matched noise study', '',
        'All three retained LS8AM positive contexts, both products and both coordinate conventions are included. '
        'No new archive bytes or exposures were acquired. TG000801_P0, TG000801_P1 and TG001401_P0 remain '
        '**UNRESOLVED_WITHIN_FIXED_SCOPE** under the original LS8AM gate.', '',
        ...
        'All 72 signed known-template controls, 120 signed compact controls and 192 additive checks pass.', '',
        '## Fixed durations, covariance and residual comparison', '',
        'TG000801_P0, TG000801_P1 and TG001401_P0 are each one 41-second exposure. The IID reference uses the exact OLS prediction weights, '
    ...
    lines += ['## Stop and interpretation boundaries', '',
        ...
        'The LS8AM classifications and LS8AL thresholds remain unchanged. No technical-origin '
        'interpretation, qualified candidate, detector or coverage is established.', '',
        'This single bounded study is closed regardless of result. Rank-14 WASP-103 is next '
        'for a separately frozen independent transfer from the unchanged target ledger, exact '
        'pair CH_PR100013_TG000101_V0300 and CH_PR100013_TG000102_V0300. Its science values '
        'remain unopened. Further HD 106315 tuning is not part of this study. Raw imagettes, '
        'reserved TESS/M43 material and the unsent calibration request remain unchanged.', '',
        '[Protocol](../LS8AN_RESIDUAL_NOISE_PROTOCOL.md) · [Full diagnostics](diagnostics.json) · '
        '[Signed injections](injection_controls.json) · [Audit](audit.json) · '
        '[Original LS8AM report](../results_ls8am_images/REPORT.md)', '']
    ...
    (OUT / 'next_action.json').write_text(json.dumps({'status': 'BOUNDED_STUDY_CLOSED',
        ...
        'next': 'Interpret complete LS8AN; then separately freeze rank-14 WASP-103 chronological-pair metadata and L2 transfer.',
        'original_ls8am_labels_preserved': True}, indent=2) + '\n')
