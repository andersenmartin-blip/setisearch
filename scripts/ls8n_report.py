#!/usr/bin/env python3
"""Readable report and figure from retained LS8N tables and audit, no network."""
import gzip
import hashlib
import json
...
OUT = ROOT/'results_ls8n_l2_screen'


def main():
    s = json.loads((OUT/'summary.json').read_text())
    a = json.loads((OUT/'audit.json').read_text())
    meta = json.loads((ROOT/'results_ls8n_l2_metadata/summary.json').read_text())
    ...
    for column, visit in enumerate(s['visits']):
        ...
        axes[0, column].set_title(visit['file_key'].split('_')[2]+' · GJ 1132')
    ...
    fig.suptitle('LS8N · two prospectively selected CHEOPS visits', fontsize=15, fontweight='bold')
    fig.savefig(OUT/'gj1132_screen.png', dpi=180)
    plt.close(fig)
    rows = []
    for v in s['visits']:
    ...
    if representatives:
        ...
        followup='Close this two-visit GJ 1132 screen as a descriptive null. The next independent cohort is rank 3 of the unchanged reconciled LS8J ledger, HD 136352. Freeze its exact pair and header preflight before accessing that cohort. Do not widen or retune the closed GJ 1132 pair.'
        reps=''
    if a['status']!='PASS':
    ...
    text=f'''# LS8N — GJ 1132 two-visit DEFAULT-L2 screen
