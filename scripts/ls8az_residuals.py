#!/usr/bin/env python3
"""LS8AZ offline use of the complete two unresolved contexts from audited LS8AY."""
import gzip
import hashlib
import json
import os
...
def main():
    ...
    for context in selected:
        ...
        header = fits.Header.fromstring((ROOT / 'results_ls8ax_l2_metadata' / key / 'lightcurve_header.bin').read_bytes().decode(), sep='')
        rows = np.frombuffer((ROOT / 'results_ls8ax_l2_screen' / key / 'lightcurve_table.bin').read_bytes(), dtype=dtype(header))[context['lo']:context['hi']]
        ...
        item = {'id': context['id'], 'sign': context['sign'],
            'original_ls8ay_classification': previous[context['id']]['classification'],
        ...
        for number in (0, 1):
            ...
            for kind in ('CAL', 'COR'):
                ...
                detail['original_ls8ay_unweighted_fits'] = previous[context['id']]['conventions'][conv]['fits'][kind]
                info[kind] = detail
    ...
    save(OUT / 'summary.json', {'status': 'COMPLETE_UNAUDITED', 'contexts': len(summaries),
        ...
        'qualified_coverage_added': False, 'original_ls8ay_classifications_unchanged': True})
