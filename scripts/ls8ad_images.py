#!/usr/bin/env python3
"""Frozen LS8AD range acquisition and paired-image analysis, with offline reuse."""
import argparse
...
def main():
    ...
    scope=json.loads((ROOT/'config/ls8ad_payload_scope.json').read_text())
    for name,digest in scope['input_pins'].items():
        assert sha((ROOT/name).read_bytes())==digest,name
    cfg=json.loads((ROOT/'config/ls8ad_images.json').read_text())
    ...
    assert [(c['id'],c['start'],c['duration'],c['lo'],c['hi']) for c in cfg['contexts']]==[
        ('TG006402_N0',21,1,7,36)]
    assert json.loads((ROOT/'results_ls8ad_metadata/audit.json').read_text())['status']=='PASS'
    for line in (ROOT/'results_ls8ad_metadata/SHA256SUMS').read_text().splitlines():
        digest,relative=line.split('  ',1)
        assert sha((ROOT/'results_ls8ad_metadata'/relative).read_bytes())==digest,relative
    ...
    for c in cfg['contexts']:
        ...
        if key not in tables:
            ...
            tables[key]=np.frombuffer((ROOT/'results_ls8ac_l2_screen'/key/'lightcurve_table.bin').read_bytes(),dtype=dtype(header))
