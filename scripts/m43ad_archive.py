...
def pack():
    ...
    data = tmp.read_bytes(); transport = base64.b64encode(data); parts = []
    for start in range(0, len(transport), 524288):
        path = ARCHIVE/f'records.jsonl.xz.b64.part{len(parts):03d}'
        value = transport[start:start+524288]; path.write_bytes(value)
        parts.append(dict(path=path.relative_to(ROOT).as_posix(), size=len(value), sha256=sha(value)))
    tmp.unlink()
    manifest = dict(format='m43ad-lossless-record-text-xz-base64-v1', transport_encoding='base64', python_version=platform.python_version(),
...
def restore(verify_only=False):
    ...
    for item in manifest['parts']:
        ...
        parts.append(data)
    packed = base64.b64decode(b''.join(parts), validate=True); assert sha(packed) == manifest['archive_sha256']
