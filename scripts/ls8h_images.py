#!/usr/bin/env python3
"""Frozen LS8H range acquisition and paired-image analysis, with offline reuse."""
import argparse
...
def main():
    ...
    cfg=json.loads((ROOT/'config/ls8h_images.json').read_text())
    ...
    for c in cfg['contexts']:
        ...
        if key not in tables:
            header=fits.Header.fromstring((ROOT/'results_ls8g_l2_metadata'/key/'lightcurve_header.bin').read_bytes().decode(),sep='')
            tables[key]=np.frombuffer((ROOT/'results_ls8g_l2_screen'/key/'lightcurve_table.bin').read_bytes(),dtype=dtype(header))
    ...
    save(OUT/'summary.json',{'status':'COMPLETE_UNAUDITED','contexts':len(results),
        ...
        'original_ls8b_audit':'FAIL','new_detector_qualification':False,'raw_imagettes_opened':False})
