#!/usr/bin/env python3
"""Transfer pinned LS8K acquisition/scoring functions to the frozen LS IV +09 2 pair."""
import argparse
import hashlib
import json
...
META = ROOT / 'results_ls8au_l2_metadata'
...
def main():
    ...
    assert not OUT.exists(), 'refuse LS8AU output overwrite'
    cfg = json.loads((ROOT / 'config/ls8au_l2_scope.json').read_text())
    ...
    summary = {'stage': 'LS8AU_LSIV092_TWO_VISIT_L2_SCREEN', 'status': 'COMPLETE_UNAUDITED',
        'evaluation_freeze_commit': args.freeze, 'target': 'LS IV +09 2', 'selected_keys': KEYS,
        ...
        'interpretation': 'Independent rank-17 two-visit L2 screen; signed endpoints are not Gaussian significances.'}
