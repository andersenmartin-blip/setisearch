#!/usr/bin/env python3
"""Transfer pinned LS8K acquisition/scoring functions to the frozen PG 1245-042 pair."""
import argparse
import hashlib
import json
...
META = ROOT / 'results_ls8y_l2_metadata'
...
def main():
    ...
    assert not OUT.exists(), 'refuse LS8Y output overwrite'
    cfg = json.loads((ROOT / 'config/ls8y_l2_scope.json').read_text())
    ...
    summary = {'stage': 'LS8Y_PG1245042_TWO_VISIT_L2_SCREEN', 'status': 'COMPLETE_UNAUDITED',
        'evaluation_freeze_commit': args.freeze, 'target': 'PG 1245-042', 'selected_keys': KEYS,
        ...
        'interpretation': 'Independent rank-7 two-visit L2 screen; signed endpoints are not Gaussian significances.'}
