#!/usr/bin/env python3
"""Readable report and figure from retained LS8AF tables and audit, no network."""
import gzip
import hashlib
import json
...
OUT = ROOT/'results_ls8af_l2_screen'


def main():
    s = json.loads((OUT/'summary.json').read_text())
    a = json.loads((OUT/'audit.json').read_text())
    meta = json.loads((ROOT/'results_ls8af_l2_metadata/summary.json').read_text())
    ...
    for column, visit in enumerate(s['visits']):
        ...
        axes[0, column].set_title(visit['file_key'].split('_')[2]+' · PG 1207-033')
    ...
    fig.suptitle('LS8AF · two prospectively selected CHEOPS visits', fontsize=15, fontweight='bold')
    fig.savefig(OUT/'pg1207033_screen.png', dpi=180)
    plt.close(fig)
    rows = []
    for v in s['visits']:
    ...
    if representatives:
        ...
        followup='Close this two-visit PG 1207-033 screen as a descriptive null. The next independent cohort is rank 11 of the unchanged reconciled LS8J ledger, EC13080-1508. Freeze its exact pair and header preflight before accessing that cohort. Do not widen or retune the closed PG 1207-033 pair.'
        reps=''
    if a['status']!='PASS':
    ...
    text=f'''# LS8AF — PG 1207-033 two-visit DEFAULT-L2 screen
...
![Retained light curves and all eligible signed scores](pg1207033_screen.png)
...
PG 1207-033 was rank 10 of the metadata-only first-1,000-public-row CHEOPS census:
