#!/usr/bin/env python3
"""Frozen LS8AR range acquisition and paired-image analysis, with offline reuse."""
import argparse
...
def main():
    ...
    scope=json.loads((ROOT/'config/ls8ar_payload_scope.json').read_text())
    ...
    assert cfg['join_rows']==814 and cfg['image_science_bytes']==260480000 and cfg['smearing_bytes']==651200
    assert [(c['id'],c['start'],c['duration'],c['lo'],c['hi']) for c in cfg['contexts']]==[('TG023701_P0', 1070, 1, 1056, 1085), ('TG023701_P1', 2033, 1, 2019, 2048), ('TG023701_P2', 2209, 1, 2195, 2224), ('TG023701_P3', 2528, 1, 2514, 2543), ('TG023701_P4', 2581, 1, 2567, 2596), ('TG023701_P5', 2761, 1, 2747, 2776), ('TG023701_P6', 2833, 1, 2819, 2848), ('TG023701_P7', 3044, 1, 3030, 3059), ('TG023701_P8', 3133, 1, 3119, 3148), ('TG023701_N0', 975, 1, 961, 990), ('TG023701_N1', 1511, 2, 1497, 1527), ('TG023701_N2', 1587, 1, 1573, 1602), ('TG023701_N3', 2679, 1, 2665, 2694), ('TG023701_N4', 3264, 1, 3250, 3279)]
    assert json.loads((ROOT/'results_ls8ar_metadata/audit.json').read_text())['status']=='PASS'
    for line in (ROOT/'results_ls8ar_metadata/SHA256SUMS').read_text().splitlines():
        digest,relative=line.split('  ',1)
        assert sha((ROOT/'results_ls8ar_metadata'/relative).read_bytes())==digest,relative
    ...
    for c in cfg['contexts']:
        ...
        if key not in tables:
            ...
            tables[key]=np.frombuffer((ROOT/'results_ls8aq_l2_screen'/key/'lightcurve_table.bin').read_bytes(),dtype=dtype(header))
