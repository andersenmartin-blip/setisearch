...
META = ROOT / 'results_ls8d_metadata'


def main():
    reps = json.loads((ROOT/'config/ls8c_auxiliary.json').read_text())['representatives']
    sources, tables, l2 = {}, {}, {}
    for key in dict.fromkeys(r[0] for r in reps):
        h = fits.Header.fromstring((ROOT/'results_ls8b_l2_metadata'/key/'lightcurve_header.bin').read_bytes().decode(), sep='')
        l2[key] = np.frombuffer((ROOT/'results_ls8b_l2_suite'/key/'lightcurve_table.bin').read_bytes(), dtype=dtype(h))
    ...
    manifest={'stage':'LS8D','status':'METADATA_JOINED_IMAGES_CLOSED','sources':sources,'contexts':contexts,
    ...
    (ROOT/'config/ls8d_images.json').write_text(json.dumps(manifest,indent=2)+'\n')
