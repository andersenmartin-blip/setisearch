#!/usr/bin/env python3
"""LS8AN offline use of the three already published, pinned LS8AM contexts."""
import gzip
import hashlib
import json
import os
...
def main():
    ...
    for context in original['contexts']:
        ...
        rows = np.frombuffer((ROOT / 'results_ls8al_l2_screen' / key / 'lightcurve_table.bin').read_bytes(), dtype=dtype(header))[context['lo']:context['hi']]
        ...
        item = {'id': context['id'], 'sign': context['sign'],
            'original_ls8am_classification': previous[context['id']]['classification'],
        ...
        for number in (0, 1):
            ...
            for kind in ('CAL', 'COR'):
                ...
                detail['original_ls8am_unweighted_fits'] = previous[context['id']]['conventions'][conv]['fits'][kind]
                info[kind] = detail
    ...
    assert payload_bytes == 55680000 and len(controls) == 192
    ...
    save(OUT / 'summary.json', {'status': 'COMPLETE_UNAUDITED', 'contexts': len(summaries),
        'actual_product_conventions': 12, 'held_duration_matched_cases': 288,
        ...
        'qualified_coverage_added': False, 'original_ls8am_classifications_unchanged': True})
