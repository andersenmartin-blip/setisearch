#!/usr/bin/env python3
"""Independent LS8P reconstruction; never imports producer modules.
...
SIDE = list(range(12)) + list(range(19, 31))
EVENT = [14, 15, 16]
BLOCKS = [list(range(s, s + 3)) for s in (0, 3, 6, 9, 19, 22, 25, 28)]
...
def reconstruct(times, cube, train, target, valid, center):
    ...
    for i in range(1, len(t)):
        segments.append(segments[-1] + int(not 30 <= t[i] - t[i - 1] <= 90))
    lags = []; counts = []
    ...
    assert h > 0 and df > 0
    info = {'rho_lag1': lags[0], 'rho_lag2': lags[1], 'lag_pair_counts': counts,
...
def boundary(maps, masks):
    ...
    for name in ('C0_only', 'C1_only'):
        ...
        result[name + '_pixels'] = {'xy': [[x, y] for y, x in coords],
...
def controls(state):
    ...
    for name, column, per_row in [('brightness', 0, .001), ('shift_x', 1, .05), ('shift_y', 2, .05)]:
        truth = np.zeros(4); truth[column] = 3 * per_row
        cases.append((name, sum(x[:, column] * per_row for _ in range(3)), truth))
    ...
    for name, plus, truth in cases:
        for sign in (-1, 1):
            ...
            records.append({'kind': name, 'sign': sign, 'exposures': 3,
...
def main():
    cfg = json.loads((ROOT / 'config/ls8p_residuals.json').read_text())
    original = json.loads((ROOT / 'config/ls8o_images.json').read_text())
    for name, digest in cfg['input_pins'].items():
        assert hashlib.sha256((ROOT / name).read_bytes()).hexdigest() == digest, name
    for line in (ROOT / 'results_ls8o_images/SHA256SUMS').read_text().splitlines():
        digest, name = line.split('  ', 1)
        assert hashlib.sha256((ROOT / 'results_ls8o_images' / name).read_bytes()).hexdigest() == digest
    actual = json.loads((OUT / 'diagnostics.json').read_text())
    previous = {v['id']: v for v in json.loads((ROOT / 'results_ls8o_images/diagnostics.json').read_text())}
    ...
    for c in original['contexts']:
        key = c['file_key']; rows = list(ROW.iter_unpack((ROOT / 'results_ls8n_l2_screen' / key / 'lightcurve_table.bin').read_bytes()))[c['lo']:c['hi']]
        times = np.array([(LD(r[2]) - LD(rows[14][2])) * 86400 for r in rows], dtype=LD)
        cubes = {}
        for kind in ('CAL', 'COR'):
            path = ROOT / 'results_ls8o_images' / c['id'] / f'SCI_{kind}_SubArray.bin.gz'
            raw = gzip.decompress(path.read_bytes()); receipt = json.loads(path.with_suffix(path.suffix + '.json').read_text())
            assert len(raw) == 9920000 and hashlib.sha256(raw).hexdigest() == receipt['raw_sha256']
            cubes[kind] = np.fromiter((r[0] for r in struct.iter_unpack('>d', raw)), dtype=float).reshape(31, 200, 200)
        ...
        item = {'id': c['id'], 'sign': c['sign'], 'original_ls8o_classification': previous[c['id']]['classification'], 'conventions': {}}
        masks = []
        with np.load(ROOT / 'results_ls8o_images' / c['id'] / 'event_maps.npz') as saved:
        ...
        for number in (0, 1):
            ...
            with np.load(OUT / c['id'] / f'{conv}_arrays.npz') as saved:
                ...
                for kind in ('CAL', 'COR'):
                    state = reconstruct(times, cubes[kind], SIDE, EVENT, valid, cc); states[kind] = state
                    ...
                    for block in BLOCKS:
                        ...
                        pseudo = reconstruct(times, cubes[kind], training, block, valid, cc)
                    ...
                    result['held_block_count'] = 8
                    result['original_ls8o_unweighted_fits'] = previous[c['id']]['conventions'][conv]['fits'][kind]
                    data[kind] = result
        ...
        references.append(item); print(c['id'], 'independent three-sum/covariance/boundary reconstruction complete', flush=True)
