#!/usr/bin/env python3
"""Frozen LS8L range acquisition and paired-image analysis, with offline reuse."""
import argparse
...
def main():
    ...
    cfg=json.loads((ROOT/'config/ls8l_images.json').read_text())
    ...
    assert [(c['id'],c['start'],c['duration'],c['lo'],c['hi']) for c in cfg['contexts']]==[
        ('TG000201_P0',521,1,507,536),('TG000202_P0',678,1,664,693),('TG000202_N0',119,1,105,134)]
    assert json.loads((ROOT/'results_ls8l_metadata/audit.json').read_text())['status']=='PASS'
    for line in (ROOT/'results_ls8l_metadata/SHA256SUMS').read_text().splitlines():
        digest,relative=line.split('  ',1)
        assert sha((ROOT/'results_ls8l_metadata'/relative).read_bytes())==digest,relative
    ...
    for c in cfg['contexts']:
        ...
        if key not in tables:
            ...
            tables[key]=np.frombuffer((ROOT/'results_ls8k_l2_screen'/key/'lightcurve_table.bin').read_bytes(),dtype=dtype(header))
