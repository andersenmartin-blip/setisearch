#!/usr/bin/env python3
"""Report every fixed LS8AM image outcome without altering the diagnostics."""
import json
import math
...
def main():
    ...
    if unresolved:
        ...
        next_action=('Close this HD 106315 follow-up under its fixed descriptive label without widening. '
                     'The next independent target is rank 14 of the unchanged reconciled LS8J ledger, WASP-103; '
                     'freeze its exact pair and header preflight before science values.')
    count=audit['comparisons']
    text=f'''# LS8AM — HD 106315 complete positive-event paired-image follow-up
