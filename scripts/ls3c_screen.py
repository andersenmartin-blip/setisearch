#!/usr/bin/env python3
"""Execute the frozen LS3C screen through the unchanged LS1 implementation."""
...
def run(config_path: Path, data_dir: Path) -> dict:
    ...
    if not config["project"]["detector_inherited_unchanged_from_ls1"]:
        raise RuntimeError("LS3C detector inheritance is not frozen")
    result = run_inherited_screen(config_path, data_dir)
    result["artifact_type"] = "seti_repeater.ls3c_medium_resolution_screen"
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("results_ls3c/screen.json"))
