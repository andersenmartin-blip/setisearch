#!/usr/bin/env python3
"""Fetch the official HD 219134 h orbit and host astrometry only."""
...
FIELDS = (
    "pl_name", "hostname", "hip_name", "hd_name", "ra", "dec", "rastr",
    "decstr", "sy_dist", "sy_plx", "sy_pmra", "sy_pmdec", "st_radv",
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("results_m16_selected_metadata/hd219134h.json"),
    )
    ...
    request = Request(url, headers={"User-Agent": "setisearch-m16-metadata/1.0"})
    ...
    result = {
        "purpose": "Milestone 16 selected-target official metadata completion",
