#!/usr/bin/env python3
"""Fixed post-hoc morphology checks for the four Milestone 18 review cases.
...
Milestone 18 arithmetic-family case together so the fixed cross-candidate
receiver-frame-alias check is applied across all four.
"""
...
SELECTED_DISPOSITIONS = (
    ("rfi_family_veto_pending_manual_review", 4),
)
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("config/gj649b_heldout_m18.json"),
    )
    parser.add_argument(
        "--summary",
        type=Path,
        default=Path("results_m18/search_summary.json"),
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
        default=Path("data_m18_candidate_investigation"),
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results_m18_candidate_investigation"),
    )
    ...
    archive = {
        ...
        "interpretation": (
            "The corrected Milestone 16 header-only screen found no second "
            "qualifying GJ 649 cadence. Any unresolved case still requires a "
            "later independent public observation."
        ),
    }
    result = {
        "analysis_label": "Milestone 18 post-hoc candidate investigation",
        "detector_status": (
