...
def main():
    verify_manifest(ROOT / "LS4F_V2_FREEZE.sha256")
    verify_manifest(ROOT / "LS4E_FREEZE.sha256")
    config = json.loads((ROOT / "config/ls4f_v2_native_reanalysis.json").read_text())
    ...
    output = ROOT / "results_ls4f_v2_reanalysis"
    if output.exists():
        raise RuntimeError("result directory exists; preserve prior execution")
    output.mkdir()
    rawdir = Path(tempfile.mkdtemp(prefix="setisearch-ls4f-", dir="/tmp"))
    if ROOT in rawdir.resolve().parents:
        raise RuntimeError("raw directory must be outside synchronized workspace")
    records = []
    try:
        ...
        result = {"artifact_type": "seti_repeater.ls4f_v2_native_reanalysis", "status": "retrospective-reanalysis-complete",
                  ...
                  "config_sha256": hashlib.sha256((ROOT / 'config/ls4f_v2_native_reanalysis.json').read_bytes()).hexdigest()}
