#!/usr/bin/env python3
"""Frozen LS7O native comparison and downstream PRF pulse protection."""
import gzip
import hashlib
import json
...
def main():
    assert not OUT.exists(),'refuse to overwrite completed LS7O results'
    cfg=read(ROOT/'config/ls7o_reference.json')
    for p,sha in cfg['input_sha256'].items():assert digest(ROOT/p)==sha,p
    for line in (ROOT/'results_ls7o_inputs/SHA256SUMS').read_text().splitlines():
        expected,name=line.split('  ',1);assert digest(ROOT/'results_ls7o_inputs'/name)==expected,name
    raw={}
    ...
    references={s:dict(np.load(ROOT/f'results_ls7o_inputs/references_s{s:03d}.npz',allow_pickle=False)) for s in [29,32]}
    metadata=read(ROOT/'results_ls7o_metadata/inventory.json')
    designs={r['sector']:np.array(r['reference_design']) for r in metadata['spatial']}
    ...
    OUT.mkdir();native=[];pulses=[];reference_rows=[]
    for wi,w in enumerate(windows):
        ...
        ref=references[s]
        try:
            estimate=affine_reference_motion(ref['centroid_xy'][:,a],ref['error_xy'][:,a],ref['quality'][:,a],designs[s],side,selected)
        except ValueError as exc:
            reference_rows.append({'window_id':w['window_id'],'sector':s,'anchor':a,'valid':False,'reason':str(exc)})
            for offset in cfg['column_offsets']:
                case=f"{w['window_id']}/offset{offset}"
                native.append({**{k:w[k] for k in ['window_id','sector','anchor','start','stop','fold_id']},
                               'case_id':case,'column_offset':offset,'reference_valid':False,'unavailable_reason':str(exc),
                               'energies':{'static':static,'motion':None,'plane':None,'combined':None}})
                for si in range(5):
                    for variant in ['nominal','minus_entries','plus_entries']:
                        pulses.append({'case_id':case+f'/pulse{si}/{variant}','window_id':w['window_id'],'sector':s,
                                       'column_offset':offset,'shift_index':si,'variant':variant,'metrics':None})
            continue
        position=estimate['positions_xy'];zero=estimate['centers_xy']
        reference_rows.append({'window_id':w['window_id'],'sector':s,'anchor':a,'valid':True,'selected':selected,
                               'positions_xy':position[selected],'formal_sigma_xy':estimate['formal_sigma_xy'][selected],
                               **{k:estimate[k] for k in ['centers_xy','sideband_median_error_xy','operators_xy','conditions_xy','reference_residual_mean_chi2_per_3_dof_xy']}})
        series={};protected=[]
        for offset in cfg['column_offsets']:
        ...
        for offset in cfg['column_offsets']:
            ...
            record={'case_id':case,'window_id':w['window_id'],'sector':s,'anchor':a,'start':lo,'stop':hi,'fold_id':w['fold_id'],
                    'column_offset':offset,'reference_valid':True,'position_reference_xy':zero,'source_fit':fit,'plane_valid':protection['valid'],
    ...
    save_rows(OUT/'reference_motion.jsonl.gz',reference_rows)
    result={'study':'LS7O six-reference affine centroid native response','source_commit':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),
            'python':platform.python_version(),'config_sha256':digest(ROOT/'config/ls7o_reference.json'),
            'reference_windows_available':sum(r['valid'] for r in reference_rows),'native_windows':420,'model_window_rows':840,'pulse_response_rows':12600,'backgrounds':20,
