#!/usr/bin/env python3
"""Readable report and figure from retained LS8AA tables and audit, no network."""
import gzip
import hashlib
import json
...
OUT = ROOT/'results_ls8aa_l2_screen'


def main():
    s = json.loads((OUT/'summary.json').read_text())
    a = json.loads((OUT/'audit.json').read_text())
    meta = json.loads((ROOT/'results_ls8aa_l2_metadata/summary.json').read_text())
    ...
    for column, visit in enumerate(s['visits']):
        ...
        axes[0, column].set_title(visit['file_key'].split('_')[2]+' · WASP-43')
    ...
    fig.suptitle('LS8AA · two prospectively selected CHEOPS visits', fontsize=15, fontweight='bold')
    fig.savefig(OUT/'wasp43_screen.png', dpi=180)
    plt.close(fig)
    rows = []
    for v in s['visits']:
    ...
    if representatives:
        ...
        followup='Close this two-visit WASP-43 screen as a descriptive null. The next independent cohort is rank 9 of the unchanged reconciled LS8J ledger, PG1303-114. Freeze its exact pair and header preflight before accessing that cohort. Do not widen or retune the closed WASP-43 pair.'
        reps=''
    if a['status']!='PASS':
    ...
    text=f'''# LS8AA — WASP-43 two-visit DEFAULT-L2 screen
...
![Retained light curves and all eligible signed scores](wasp43_screen.png)
...
WASP-43 was rank 8 of the metadata-only first-1,000-public-row CHEOPS census:
...
Image bytes are zero; no alternative aperture, raw imagette or later WASP-43
visit was opened.
