#!/usr/bin/env python3
"""Report every frozen LS8AH outcome after independent verification."""
import json
...
def main():
    ...
    lines = ['# LS8AH: PG 1207-033 residual and duration-matched noise study', '',
        'The retained LS8AG positive context, both products and both coordinate conventions are included. '
        'No new archive bytes or exposures were acquired. TG000901_P0 remains '
        '**UNRESOLVED_WITHIN_FIXED_SCOPE** under the original LS8AG gate.', '',
        ...
        'TG000901_P0 is one 60-second exposure. The IID reference uses the exact OLS prediction weights, '
        ...
        'The positive context has 24 single-row control targets. Each excludes two neighboring rows on each side '
    ...
    lines += ['## Stop and interpretation boundaries', '',
        ...
        'The LS8AG classifications and LS8AF thresholds remain unchanged. No technical-origin '
        'interpretation, qualified candidate, detector or coverage is established.', '',
        'This single bounded study is closed regardless of result. Rank-11 EC13080-1508 is next '
        'for a separately frozen independent transfer from the unchanged target ledger, exact '
        'pair CH_PR100002_TG005201_V0300 and CH_PR100002_TG005202_V0300. Its science values '
        'remain unopened. Further PG 1207-033 tuning is not part of this study. Raw imagettes, '
        'reserved TESS/M43 material and the unsent calibration request remain unchanged.', '',
        '[Protocol](../LS8AH_RESIDUAL_NOISE_PROTOCOL.md) · [Full diagnostics](diagnostics.json) · '
        '[Signed injections](injection_controls.json) · [Audit](audit.json) · '
        '[Original LS8AG report](../results_ls8ag_images/REPORT.md)', '']
    ...
    (OUT / 'next_action.json').write_text(json.dumps({'status': 'BOUNDED_STUDY_CLOSED',
        ...
        'next': 'Interpret complete LS8AH; then separately freeze rank-11 EC13080-1508 chronological-pair metadata and L2 transfer.',
        'original_ls8ag_labels_preserved': True}, indent=2) + '\n')
