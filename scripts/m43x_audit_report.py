...
def main():
    ...
    for rec in ledger:
        ...
        if len(rec['case']['active_epochs'])==2:assert finals['epoch_confirmation']==finals['remaining_aggregate']
    payload_groups={}
    for rec in ledger:
    ...
    lines+=['Every lost associated member and its exact epoch evidence is retained in '+
        ...
        'The two-epoch aggregate rule is mathematically identical to the hard minimum rule; all final sets verify that identity. '+
