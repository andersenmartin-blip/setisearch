#!/usr/bin/env python3
"""Independent LS8AS reconstruction; never imports producer modules.
...
def main():
    cfg = json.loads((ROOT / 'config/ls8as_residuals.json').read_text())
    original = json.loads((ROOT / 'config/ls8ar_images.json').read_text())
    for name, digest in cfg['input_pins'].items():
        assert hashlib.sha256((ROOT / name).read_bytes()).hexdigest() == digest, name
    for line in (ROOT / 'results_ls8ar_images/SHA256SUMS').read_text().splitlines():
        digest, name = line.split('  ', 1)
        assert hashlib.sha256((ROOT / 'results_ls8ar_images' / name).read_bytes()).hexdigest() == digest
    actual = json.loads((OUT / 'diagnostics.json').read_text())
    previous = {v['id']: v for v in json.loads((ROOT / 'results_ls8ar_images/diagnostics.json').read_text())}
    assert cfg['all_image_context_ids'] == [c['id'] for c in original['contexts']] and len(original['contexts']) == 14
    assert cfg['original_classifications'] == {k: v['classification'] for k, v in previous.items()}
    selected = [c for c in original['contexts'] if previous[c['id']]['classification'] == 'UNRESOLVED_WITHIN_FIXED_SCOPE']
    assert cfg['context_ids'] == [c['id'] for c in selected] == ['TG023701_P3', 'TG023701_P5']
    ...
    for c in selected:
        ...
        key = c['file_key']; rows = list(ROW.iter_unpack((ROOT / 'results_ls8aq_l2_screen' / key / 'lightcurve_table.bin').read_bytes()))[c['lo']:c['hi']]
        times = np.array([(LD(r[2]) - LD(rows[14][2])) * 86400 for r in rows], dtype=LD)
        cubes = {}
        for kind in ('CAL', 'COR'):
            path = ROOT / 'results_ls8ar_images' / c['id'] / f'SCI_{kind}_SubArray.bin.gz'
        ...
        item = {'id': c['id'], 'sign': c['sign'], 'original_ls8ar_classification': previous[c['id']]['classification'],
            'duration_rows': duration, 'cadence_seconds': cadence, 'event_rows': EVENT, 'conventions': {}}
        masks = []
        with np.load(ROOT / 'results_ls8ar_images' / c['id'] / 'event_maps.npz') as saved:
        ...
        for number in (0, 1):
            ...
            with np.load(OUT / c['id'] / f'{conv}_arrays.npz') as saved:
                ...
                for kind in ('CAL', 'COR'):
                    ...
                    result['original_ls8ar_unweighted_fits'] = previous[c['id']]['conventions'][conv]['fits'][kind]
                    data[kind] = result
    ...
    compare(json.loads((OUT / 'summary.json').read_text()), {
        'status': 'COMPLETE_UNAUDITED', 'contexts': 2, 'actual_product_conventions': 8,
        'held_duration_matched_cases': 192, 'signed_injection_cases': 128,
        'retained_cal_cor_bytes_read': 37120000, 'new_native_source_bytes': 0,
        'new_classifier': False, 'qualified_candidates_added': 0, 'qualified_coverage_added': False,
        'original_ls8ar_classifications_unchanged': True}, 'summary')
