#!/usr/bin/env python3
"""Transfer pinned LS8K acquisition/scoring functions to the frozen GJ 494 pair."""
import argparse
import hashlib
import json
...
def main():
    ...
    assert not OUT.exists(), 'refuse LS8BD output overwrite'
    cfg = json.loads((ROOT / 'config/ls8bd_l2_scope.json').read_text())
    ...
    summary = {'stage': 'LS8BD_GJ494_TWO_VISIT_L2_SCREEN', 'status': 'COMPLETE_UNAUDITED',
        'evaluation_freeze_commit': args.freeze, 'target': 'GJ 494', 'selected_keys': KEYS,
        ...
        'interpretation': 'Independent rank-23 two-visit L2 screen; signed endpoints are not Gaussian significances.'}
