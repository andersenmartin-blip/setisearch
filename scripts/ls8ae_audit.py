#!/usr/bin/env python3
"""Independent LS8AE reconstruction; never imports producer modules.
...
def main():
    cfg = json.loads((ROOT / 'config/ls8ae_residuals.json').read_text())
    original = json.loads((ROOT / 'config/ls8ad_images.json').read_text())
    for name, digest in cfg['input_pins'].items():
        assert hashlib.sha256((ROOT / name).read_bytes()).hexdigest() == digest, name
    for line in (ROOT / 'results_ls8ad_images/SHA256SUMS').read_text().splitlines():
        digest, name = line.split('  ', 1)
        assert hashlib.sha256((ROOT / 'results_ls8ad_images' / name).read_bytes()).hexdigest() == digest
    actual = json.loads((OUT / 'diagnostics.json').read_text())
    previous = {v['id']: v for v in json.loads((ROOT / 'results_ls8ad_images/diagnostics.json').read_text())}
    ...
    for c in original['contexts']:
        ...
        key = c['file_key']; rows = list(ROW.iter_unpack((ROOT / 'results_ls8ac_l2_screen' / key / 'lightcurve_table.bin').read_bytes()))[c['lo']:c['hi']]
        times = np.array([(LD(r[2]) - LD(rows[14][2])) * 86400 for r in rows], dtype=LD)
        cubes = {}
        for kind in ('CAL', 'COR'):
            path = ROOT / 'results_ls8ad_images' / c['id'] / f'SCI_{kind}_SubArray.bin.gz'
        ...
        item = {'id': c['id'], 'sign': c['sign'], 'original_ls8ad_classification': previous[c['id']]['classification'],
            'duration_rows': duration, 'cadence_seconds': cadence, 'event_rows': EVENT, 'conventions': {}}
        masks = []
        with np.load(ROOT / 'results_ls8ad_images' / c['id'] / 'event_maps.npz') as saved:
        ...
        for number in (0, 1):
            ...
            with np.load(OUT / c['id'] / f'{conv}_arrays.npz') as saved:
                ...
                for kind in ('CAL', 'COR'):
                    ...
                    result['original_ls8ad_unweighted_fits'] = previous[c['id']]['conventions'][conv]['fits'][kind]
                    data[kind] = result
    ...
    compare(json.loads((OUT / 'summary.json').read_text()), {
        ...
        'original_ls8ad_classifications_unchanged': True}, 'summary')
