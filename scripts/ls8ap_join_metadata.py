...
META = ROOT / 'results_ls8ap_metadata'


def main():
    reps = json.loads((ROOT/'config/ls8ap_representatives.json').read_text())['representatives']
    expected=[]
    for key in ['CH_PR100013_TG000101_V0300','CH_PR100013_TG000102_V0300']:
    ...
    for key in dict.fromkeys(r[0] for r in reps):
        h = fits.Header.fromstring((ROOT/'results_ls8ao_l2_metadata'/key/'lightcurve_header.bin').read_bytes().decode(), sep='')
        l2[key] = np.frombuffer((ROOT/'results_ls8ao_l2_screen'/key/'lightcurve_table.bin').read_bytes(), dtype=dtype(h))
    ...
    manifest={'stage':'LS8AP','status':'METADATA_JOINED_IMAGES_CLOSED','sources':sources,'contexts':contexts,
    ...
    (ROOT/'config/ls8ap_images.json').write_text(json.dumps(manifest,indent=2)+'\n')
