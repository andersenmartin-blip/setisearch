#!/usr/bin/env python3
"""Report every frozen LS8AZ outcome after independent verification."""
import json
...
OUT = ROOT / 'results_ls8az_residuals'
...
def main():
    ...
    lines = ['# LS8AZ: GJ 536 residual and duration-matched noise study', '',
        'Both unresolved LS8AY positive contexts, both products and both conventions are included. The other seven representatives retain their closed image labels without additional native fitting. '
        'No new archive bytes or exposures were acquired. TG023501_P1 and TG023501_P6 remain '
        '**UNRESOLVED_WITHIN_FIXED_SCOPE** under the original LS8AY gate.', '',
        ...
        'TG023501_P1 and TG023501_P6 are each one 40.1699981689453-second exposure. The IID reference uses the exact OLS prediction weights, '
        ...
        'All event/baseline covariance enters the same quadratic form for the native and held rows. '
        ...
        'Each positive context has 24 single-row control targets. Each excludes two neighboring rows on each side '
    ...
    lines += ['', 'Pure brightness additions are ±0.1% per exposure: summed coefficients '
        'are ±0.001 for each one-row context. Displacement additions '
    ...
    lines += ['## Stop and interpretation boundaries', '',
        ...
        'The LS8AY classifications and LS8AX thresholds remain unchanged. No technical-origin '
        'interpretation, qualified candidate, detector or coverage is established.', '',
        'This single bounded study is closed regardless of result. Rank-21 2MASS J11285624+1010395 is next '
        'for a separately frozen independent transfer from the unchanged target ledger, exact '
        'pair CH_PR100018_TG010801_V0300 and CH_PR100018_TG010802_V0300. Its science values '
        'remain unopened. Further GJ 536 tuning is not part of this study. Raw imagettes, '
        'reserved TESS/M43 material and the unsent calibration request remain unchanged.', '',
        '[Protocol](../LS8AZ_RESIDUAL_NOISE_PROTOCOL.md) · [Full diagnostics](diagnostics.json) · '
        '[Signed injections](injection_controls.json) · [Audit](audit.json) · '
        '[Original LS8AY report](../results_ls8ay_images/REPORT.md)', '']
    ...
    (OUT / 'next_action.json').write_text(json.dumps({'status': 'BOUNDED_STUDY_CLOSED',
        ...
        'next': 'Interpret complete LS8AZ; then separately freeze rank-21 2MASS J11285624+1010395 chronological-pair metadata and L2 transfer.',
        'original_ls8ay_labels_preserved': True}, indent=2) + '\n')
