#!/usr/bin/env python3
"""Transfer pinned LS8K acquisition/scoring functions to the frozen HD 136352 pair."""
import argparse
import hashlib
import json
...
META = ROOT / 'results_ls8q_l2_metadata'
...
def main():
    ...
    assert not OUT.exists(), 'refuse LS8Q output overwrite'
    cfg = json.loads((ROOT / 'config/ls8q_l2_scope.json').read_text())
    ...
    summary = {'stage': 'LS8Q_HD136352_TWO_VISIT_L2_SCREEN', 'status': 'COMPLETE_UNAUDITED',
        'evaluation_freeze_commit': args.freeze, 'target': 'HD 136352', 'selected_keys': KEYS,
        ...
        'interpretation': 'Independent rank-3 two-visit L2 screen; signed endpoints are not Gaussian significances.'}
