#!/usr/bin/env python3
"""Report every fixed LS8L image outcome without altering the diagnostics."""
import json
import math
...
def main():
    ...
    if unresolved:
        ...
        next_action=('Close all three WASP-189 follow-up branches under their fixed descriptive labels without widening. '
                     'The next independent target is rank 2 of the unchanged reconciled LS8J ledger, GJ 1132; '
                     'freeze its exact pair and header preflight before science values.')
    count=audit['comparisons']
    text=f'''# LS8L — all three WASP-189 paired-image follow-ups
...
the negative control. Its outcomes are: **{json.dumps(summary['outcomes'],sort_keys=True)}**.
