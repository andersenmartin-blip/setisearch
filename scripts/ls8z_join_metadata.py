...
META = ROOT / 'results_ls8z_metadata'


def main():
    reps = json.loads((ROOT/'config/ls8z_representatives.json').read_text())['representatives']
    expected=[]
    for key in ['CH_PR100002_TG008601_V0300','CH_PR100002_TG008602_V0300']:
        clusters=json.loads((ROOT/'results_ls8y_l2_screen'/key/'clusters.json').read_text())
    ...
    for key in dict.fromkeys(r[0] for r in reps):
        h = fits.Header.fromstring((ROOT/'results_ls8y_l2_metadata'/key/'lightcurve_header.bin').read_bytes().decode(), sep='')
        l2[key] = np.frombuffer((ROOT/'results_ls8y_l2_screen'/key/'lightcurve_table.bin').read_bytes(), dtype=dtype(h))
    ...
    manifest={'stage':'LS8Z','status':'METADATA_JOINED_IMAGES_CLOSED','sources':sources,'contexts':contexts,
    ...
    (ROOT/'config/ls8z_images.json').write_text(json.dumps(manifest,indent=2)+'\n')
