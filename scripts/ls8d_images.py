#!/usr/bin/env python3
"""Frozen LS8D range acquisition and paired-image analysis, with offline reuse."""
import argparse
...
def main():
    ...
    cfg=json.loads((ROOT/'config/ls8d_images.json').read_text())
    ...
    for c in cfg['contexts']:
        ...
        if key not in tables:
            header=fits.Header.fromstring((ROOT/'results_ls8b_l2_metadata'/key/'lightcurve_header.bin').read_bytes().decode(),sep='')
            tables[key]=np.frombuffer((ROOT/'results_ls8b_l2_suite'/key/'lightcurve_table.bin').read_bytes(),dtype=dtype(header))
