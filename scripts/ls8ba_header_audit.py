#!/usr/bin/env python3
"""Independent scalar FITS-card audit of retained LS8BA headers; no network."""
import hashlib
import json
import struct
...
META = ROOT / 'results_ls8ba_l2_metadata'
OUT = ROOT / 'verification_ls8ba'
...
def main():
    cfg = json.loads((ROOT / 'config/ls8ba_selected_pair.json').read_text())
    ...
    for visit, p in zip(cfg['cohort']['selected_visits'], suite['products'], strict=True):
        ...
        exp = float(table.get('EXPTIME', primary.get('EXPTIME')))
        total_exp = float(table.get('TEXPTIME', primary.get('TEXPTIME')))
        assert exp == total_exp and exp > 0
        expected_hex = {'CH_PR100018_TG010801_V0300': '42700000',
                        'CH_PR100018_TG010802_V0300': '42700000'}[key]
        for actual, ledger in [(exp, float(visit['obs_exptime'])),
                               (total_exp, float(visit['obs_total_exptime']))]:
            assert struct.pack('>f', actual).hex() == expected_hex
            assert struct.pack('>f', ledger).hex() == expected_hex
        assert cfg['exposure_verification']['binary32_big_endian_hex_by_key'][key] == expected_hex
        ...
        products.append({'file_key': key, 'rows': p['rows'], 'row_bytes': 138,
                         ...
                         'exposure_tuple': [1, exp, total_exp], 'ledger_binary32_hex_verified': expected_hex, 'identity_and_receipts_verified': True})
