#!/usr/bin/env python3
"""Report every frozen LS8AE outcome after independent verification."""
import json
...
OUT = ROOT / 'results_ls8ae_residuals'
...
def main():
    ...
    lines = ['# LS8AE: PG1303-114 residual and duration-matched noise study', '',
        'The retained LS8AD negative context, both products and both coordinate conventions are included. '
        'No new archive bytes or exposures were acquired. TG006402_N0 remains '
        '**UNRESOLVED_WITHIN_FIXED_SCOPE** under the original LS8AD gate.', '',
        ...
        'All 24 signed known-template controls, 40 signed compact controls and 64 additive checks pass.', '',
        '## Fixed durations, covariance and residual comparison', '',
        'TG006402_N0 is one 60-second exposure. The IID reference uses the exact OLS prediction weights, '
        ...
        'The negative context has 24 single-row control targets. Each excludes two neighboring rows on each side '
    ...
    lines += ['', 'Pure brightness additions are ±0.1% per exposure: summed coefficients '
        'are ±0.001 for this one-row context. Displacement additions '
    ...
    lines += ['## Stop and interpretation boundaries', '',
        ...
        'The LS8AD classifications and LS8AC thresholds remain unchanged. No technical-origin '
        'interpretation, qualified candidate, detector or coverage is established.', '',
        'This single bounded study is closed regardless of result. Rank-10 PG 1207-033 is next '
        'for a separately frozen independent transfer from the unchanged target ledger, exact '
        'pair CH_PR100002_TG000901_V0300 and CH_PR100002_TG000902_V0300. Its science values '
        'remain unopened. Further PG1303-114 tuning is not part of this study. Raw imagettes, '
        'reserved TESS/M43 material and the unsent calibration request remain unchanged.', '',
        '[Protocol](../LS8AE_RESIDUAL_NOISE_PROTOCOL.md) · [Full diagnostics](diagnostics.json) · '
        '[Signed injections](injection_controls.json) · [Audit](audit.json) · '
        '[Original LS8AD report](../results_ls8ad_images/REPORT.md)', '']
    ...
    (OUT / 'next_action.json').write_text(json.dumps({'status': 'BOUNDED_STUDY_CLOSED',
        ...
        'next': 'Interpret complete LS8AE; then separately freeze rank-10 PG 1207-033 chronological-pair metadata and L2 transfer.',
        'original_ls8ad_labels_preserved': True}, indent=2) + '\n')
