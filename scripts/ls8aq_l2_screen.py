#!/usr/bin/env python3
"""Transfer pinned LS8K acquisition/scoring functions to the frozen GJ 581 pair."""
import argparse
import hashlib
import json
...
META = ROOT / 'results_ls8aq_l2_metadata'
...
def main():
    ...
    assert not OUT.exists(), 'refuse LS8AQ output overwrite'
    cfg = json.loads((ROOT / 'config/ls8aq_l2_scope.json').read_text())
    ...
    summary = {'stage': 'LS8AQ_GJ581_TWO_VISIT_L2_SCREEN', 'status': 'COMPLETE_UNAUDITED',
        'evaluation_freeze_commit': args.freeze, 'target': 'GJ 581', 'selected_keys': KEYS,
        ...
        'interpretation': 'Independent rank-15 two-visit L2 screen; signed endpoints are not Gaussian significances.'}
