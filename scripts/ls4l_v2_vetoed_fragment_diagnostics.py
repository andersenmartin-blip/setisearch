#!/usr/bin/env python3
"""LS4L v2: identical analysis, canonical replay and prevalidation persistence."""
import gzip
import hashlib
import json
...
OUT=ROOT/'results_ls4l_v2_vetoed_fragment_diagnostics'
...
def canonical_replay_agreement(actual,expected,tolerance):
    return numeric_agreement(json.loads(encoded(actual)),expected,**tolerance)
...
def main():
    ...
    rawdir=Path(tempfile.mkdtemp(prefix='setisearch-ls4l-v2-',dir='/tmp'))
    receipts=[];series={};charged=config['prior_charged_download_bytes']
    try:
        ...
        # Persist all derived measurements before any post-execution validation.
        (OUT/'prevalidation.json.gz').write_bytes(gzip.compress(encoded(result),mtime=0))
        previous=load('results_ls4i_measured_digital_injections/injections.json.gz')
        if not canonical_replay_agreement(result['fixed_window_diagnostics'],previous['fixed_window_diagnostics'],config['replay_tolerance']):
        ...
        result.update({'status':'veto-preserving-measured-diagnostic-complete','execution_version':2,'fixed_window_replay_agrees':True,
                       'stage1_result_sha256':config['stage1_result_sha256'],
                       'freeze_sha256':hashlib.sha256((ROOT/'LS4L_V2_FREEZE.sha256').read_bytes()).hexdigest(),
        ...
        summary={'status':result['status'],'paired_configurations':144,'selected_fragment_evaluations':256,
                 ...
                 'execution_version':2,'total_charged_bytes_including_v1':charged}
