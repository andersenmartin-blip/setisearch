#!/usr/bin/env python3
"""Frozen LS8R range acquisition and paired-image analysis, with offline reuse."""
import argparse
...
def main():
    ...
    scope=json.loads((ROOT/'config/ls8r_payload_scope.json').read_text())
    for name,digest in scope['input_pins'].items():
        assert sha((ROOT/name).read_bytes())==digest,name
    cfg=json.loads((ROOT/'config/ls8r_images.json').read_text())
    assert cfg['status']=='METADATA_JOINED_IMAGES_CLOSED' and cfg['image_pixels_read']==0
    assert cfg['join_rows']==176 and cfg['image_science_bytes']==56320000 and cfg['smearing_bytes']==140800
    assert [(c['id'],c['start'],c['duration'],c['lo'],c['hi']) for c in cfg['contexts']]==[
        ('TG000901_N0',46,1,32,61),('TG000901_N1',196,1,182,211),('TG000101_N0',432,2,418,448)]
    assert json.loads((ROOT/'results_ls8r_metadata/audit.json').read_text())['status']=='PASS'
    for line in (ROOT/'results_ls8r_metadata/SHA256SUMS').read_text().splitlines():
        digest,relative=line.split('  ',1)
        assert sha((ROOT/'results_ls8r_metadata'/relative).read_bytes())==digest,relative
    ...
    for c in cfg['contexts']:
        ...
        if key not in tables:
            header=fits.Header.fromstring((ROOT/'results_ls8q_l2_metadata'/key/'lightcurve_header.bin').read_bytes().decode(),sep='')
            tables[key]=np.frombuffer((ROOT/'results_ls8q_l2_screen'/key/'lightcurve_table.bin').read_bytes(),dtype=dtype(header))
