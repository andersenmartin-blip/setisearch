#!/usr/bin/env python3
"""Readable report and figure from retained LS8AC tables and audit, no network."""
import gzip
import hashlib
import json
...
OUT = ROOT/'results_ls8ac_l2_screen'


def main():
    s = json.loads((OUT/'summary.json').read_text())
    a = json.loads((OUT/'audit.json').read_text())
    meta = json.loads((ROOT/'results_ls8ac_l2_metadata/summary.json').read_text())
    ...
    for column, visit in enumerate(s['visits']):
        ...
        axes[0, column].set_title(visit['file_key'].split('_')[2]+' · PG1303-114')
    ...
    fig.suptitle('LS8AC · two prospectively selected CHEOPS visits', fontsize=15, fontweight='bold')
    fig.savefig(OUT/'pg1303114_screen.png', dpi=180)
    plt.close(fig)
    rows = []
    for v in s['visits']:
    ...
    if representatives:
        ...
        followup='Close this two-visit PG1303-114 screen as a descriptive null. The next independent cohort is rank 10 of the unchanged reconciled LS8J ledger, PG 1207-033. Freeze its exact pair and header preflight before accessing that cohort. Do not widen or retune the closed PG1303-114 pair.'
        reps=''
    if a['status']!='PASS':
    ...
    text=f'''# LS8AC — PG1303-114 two-visit DEFAULT-L2 screen

Completed 22 September 2026. Result: **{s['status']}**.
...
![Retained light curves and all eligible signed scores](pg1303114_screen.png)
...
PG1303-114 was rank 9 of the metadata-only first-1,000-public-row CHEOPS census:
...
Image bytes are zero; no alternative aperture, raw imagette or later PG1303-114
visit was opened.
