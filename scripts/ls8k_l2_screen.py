#!/usr/bin/env python3
"""Evaluate the frozen two-visit LS8K WASP-189 DEFAULT-L2 screen."""
...
META = ROOT / "results_ls8k_l2_metadata"
...
def main() -> None:
    ...
    assert not OUT.exists(), "refuse completed LS8K screen overwrite"

    for line in (META / "SHA256SUMS").read_text().splitlines():
        expected, relative = line.split("  ", 1)
        assert sha((META / relative).read_bytes()) == expected, relative
    ...
    assert [(by_key[k]['rows'], by_key[k]['row_bytes'],
             by_key[k]['table_data_start'], by_key[k]['table_bytes_declared'])
            for k in KEYS] == [(771, 138, 20160, 106398), (833, 138, 20160, 114954)]

    OUT.mkdir()
    results = [evaluate_visit(key, by_key[key]) for key in KEYS]
    summary = {
        "stage": "LS8K_WASP189_TWO_VISIT_L2_SCREEN",
