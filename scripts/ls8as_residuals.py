#!/usr/bin/env python3
"""LS8AS offline use of the complete two unresolved contexts from audited LS8AR."""
import gzip
import hashlib
import json
import os
...
def main():
    ...
    for context in selected:
        ...
        header = fits.Header.fromstring((ROOT / 'results_ls8aq_l2_metadata' / key / 'lightcurve_header.bin').read_bytes().decode(), sep='')
        rows = np.frombuffer((ROOT / 'results_ls8aq_l2_screen' / key / 'lightcurve_table.bin').read_bytes(), dtype=dtype(header))[context['lo']:context['hi']]
        ...
        item = {'id': context['id'], 'sign': context['sign'],
            'original_ls8ar_classification': previous[context['id']]['classification'],
        ...
        for number in (0, 1):
            ...
            for kind in ('CAL', 'COR'):
                ...
                detail['original_ls8ar_unweighted_fits'] = previous[context['id']]['conventions'][conv]['fits'][kind]
                info[kind] = detail
    ...
    assert payload_bytes == 37120000 and len(controls) == 128
    ...
    save(OUT / 'summary.json', {'status': 'COMPLETE_UNAUDITED', 'contexts': len(summaries),
        'actual_product_conventions': 8, 'held_duration_matched_cases': 192,
        ...
        'qualified_coverage_added': False, 'original_ls8ar_classifications_unchanged': True})
