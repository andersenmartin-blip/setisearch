#!/usr/bin/env python3
"""Report every fixed LS8AD image outcome without altering the diagnostics."""
import json
import math
...
def main():
    ...
    if unresolved:
        ...
        next_action=('Close this PG1303-114 follow-up under its fixed descriptive label without widening. '
                     'The next independent target is rank 10 of the unchanged reconciled LS8J ledger, PG 1207-033; '
                     'freeze its exact pair and header preflight before science values.')
    count=audit['comparisons']
    text=f'''# LS8AD — PG1303-114 negative-event paired-image follow-up
