#!/usr/bin/env python3
"""Independent LS7N rebuild: raw FITS, SciPy interpolation, QR/normal equations.

No imports from the LS7N producer, LS7N operator or LS7M PRF operator.
"""
import gzip
import hashlib
import json
...
def main():
    ...
    inherited={p:verify_manifest(ROOT/p) for p in ['results_ls7k_inputs','results_ls7l_engineering','results_ls7l_inputs','results_ls7m_prf_inputs','results_ls7m_response','results_ls7j_auxiliary']}
    ...
    pos={s:np.load(ROOT/f'results_ls7j_auxiliary/auxiliary_s{s:03d}.npz',allow_pickle=False)['position_yx'][...,::-1] for s in [29,32]}
    ...
    for wi,w in enumerate(windows):
        ...
        ref=np.median(cube[side],axis=0);zero=np.median(pos[s][a,side],axis=0);displacement=pos[s][a]-zero
    ...
    audit={'status':'PASS','native_windows':420,'model_window_rows':840,'pulse_response_rows':12600,
           ...
           'independent_joint_feasibility_pass':joint,'method':'raw FITS + SciPy interpolation + normal source equations + independent SVD/QR plane + scalar quadratic energies'}
