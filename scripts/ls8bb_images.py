#!/usr/bin/env python3
"""Frozen LS8BB range acquisition and paired-image analysis, with offline reuse."""
import argparse
...
def main():
    ...
    scope=json.loads((ROOT/'config/ls8bb_payload_scope.json').read_text())
    ...
    assert cfg['join_rows']==116 and cfg['image_science_bytes']==37120000 and cfg['smearing_bytes']==92800
    assert [(c['id'],c['start'],c['duration'],c['lo'],c['hi']) for c in cfg['contexts']]==[('TG010801_P0',25,1,11,40),('TG010801_N0',24,1,10,39)]
    assert json.loads((ROOT/'results_ls8bb_metadata/audit.json').read_text())['status']=='PASS'
    for line in (ROOT/'results_ls8bb_metadata/SHA256SUMS').read_text().splitlines():
        digest,relative=line.split('  ',1)
        assert sha((ROOT/'results_ls8bb_metadata'/relative).read_bytes())==digest,relative
    ...
    for c in cfg['contexts']:
        ...
        if key not in tables:
            header=fits.Header.fromstring((ROOT/'results_ls8ba_l2_metadata'/key/'lightcurve_header.bin').read_bytes().decode(),sep='')
            tables[key]=np.frombuffer((ROOT/'results_ls8ba_l2_screen'/key/'lightcurve_table.bin').read_bytes(),dtype=dtype(header))
