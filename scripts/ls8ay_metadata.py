#!/usr/bin/env python3
"""Acquire only named CAL/COR headers and exposure metadata for LS8AY."""
...
OUT = ROOT / 'results_ls8ay_metadata'
SEARCH_KEYS = ['CH_PR100011_TG023501_V0300', 'CH_PR100018_TG007801_V0300']
KEYS = ['CH_PR100011_TG023501_V0300', 'CH_PR100018_TG007801_V0300']
...
def main():
    ...
    cfg=json.loads((ROOT/'config/ls8ay_representatives.json').read_text())
    for name,digest in cfg['input_pins'].items():
        assert hashlib.sha256((ROOT/name).read_bytes()).hexdigest()==digest,name
    assert json.loads((ROOT/'results_ls8ax_l2_screen/audit.json').read_text())['status']=='PASS'
    for line in (ROOT/'results_ls8ax_l2_screen/SHA256SUMS').read_text().splitlines():
        digest,name=line.split('  ',1)
        assert hashlib.sha256((ROOT/'results_ls8ax_l2_screen'/name).read_bytes()).hexdigest()==digest,name
    assert cfg['search_keys']==SEARCH_KEYS and cfg['metadata_keys']==KEYS
    expected=[]
    for key in SEARCH_KEYS:
        clusters=json.loads((ROOT/'results_ls8ax_l2_screen'/key/'clusters.json').read_text())
        for sign in ('positive','negative'):
            for c in clusters[sign]:
                rep=c['representative'];expected.append([key,sign,c['cluster_id'],rep['start'],rep['duration']])
    assert cfg['representatives']==expected and len(expected)==9
    assert sum(28+r[4] for r in expected)==cfg['expected_context_rows']==261
    assert cfg['expected_cal_cor_joins']==522
    assert not OUT.exists(), 'refuse completed LS8AY metadata overwrite'
    OUT.mkdir()
