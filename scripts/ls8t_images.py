#!/usr/bin/env python3
"""Frozen LS8T range acquisition and paired-image analysis, with offline reuse."""
import argparse
...
def main():
    ...
    scope=json.loads((ROOT/'config/ls8t_payload_scope.json').read_text())
    for name,digest in scope['input_pins'].items():
        assert sha((ROOT/name).read_bytes())==digest,name
    cfg=json.loads((ROOT/'config/ls8t_images.json').read_text())
    assert cfg['status']=='METADATA_JOINED_IMAGES_CLOSED' and cfg['image_pixels_read']==0
    assert cfg['join_rows']==120 and cfg['image_science_bytes']==38400000 and cfg['smearing_bytes']==96000
    assert [(c['id'],c['start'],c['duration'],c['lo'],c['hi']) for c in cfg['contexts']]==[
        ('TG000101_N0',317,3,303,334),('TG015701_P0',217,1,203,232)]
    assert json.loads((ROOT/'results_ls8t_metadata/audit.json').read_text())['status']=='PASS'
    for line in (ROOT/'results_ls8t_metadata/SHA256SUMS').read_text().splitlines():
        digest,relative=line.split('  ',1)
        assert sha((ROOT/'results_ls8t_metadata'/relative).read_bytes())==digest,relative
    ...
    for c in cfg['contexts']:
        ...
        if key not in tables:
            header=fits.Header.fromstring((ROOT/'results_ls8s_l2_metadata'/key/'lightcurve_header.bin').read_bytes().decode(),sep='')
            tables[key]=np.frombuffer((ROOT/'results_ls8s_l2_screen'/key/'lightcurve_table.bin').read_bytes(),dtype=dtype(header))
