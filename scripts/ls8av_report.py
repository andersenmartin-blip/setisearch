#!/usr/bin/env python3
"""Readable report and figure from retained LS8AV tables and audit, no network."""
import gzip
import hashlib
import json
...
OUT = ROOT/'results_ls8av_l2_screen'


def main():
    s = json.loads((OUT/'summary.json').read_text())
    a = json.loads((OUT/'audit.json').read_text())
    meta = json.loads((ROOT/'results_ls8av_l2_metadata/summary.json').read_text())
    ...
    for column, visit in enumerate(s['visits']):
        ...
        axes[0, column].set_title(visit['file_key'].split('_')[2]+' · GJ 9404')
    ...
    fig.suptitle('LS8AV · two prospectively selected CHEOPS visits', fontsize=15, fontweight='bold')
    fig.savefig(OUT/'gj9404_screen.png', dpi=180)
    plt.close(fig)
    rows = []
    for v in s['visits']:
    ...
    if representatives:
        ...
        followup='Close this two-visit GJ 9404 screen as a descriptive null. The next independent cohort is rank 19 of the unchanged reconciled LS8J ledger, EC14338-1445. Freeze its exact pair and header preflight before accessing that cohort. Do not widen or retune the closed GJ 9404 pair.'
        reps=''
    if a['status']!='PASS':
    ...
    text=f'''# LS8AV — GJ 9404 two-visit DEFAULT-L2 screen
...
![Retained light curves and all eligible signed scores](gj9404_screen.png)
...
GJ 9404 was rank 18 of the metadata-only first-1,000-public-row CHEOPS census:
