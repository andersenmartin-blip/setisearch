#!/usr/bin/env python3
"""Report every fixed LS8X image outcome without altering the diagnostics."""
import json
import math
...
def main():
    ...
    if unresolved:
        ...
        next_action=('Close this GJ 436 follow-up under its fixed descriptive label without widening. '
                     'The next independent target is rank 7 of the unchanged reconciled LS8J ledger, PG 1245-042; '
                     'freeze its exact pair and header preflight before science values.')
    count=audit['comparisons']
    text=f'''# LS8X — GJ 436 positive-event paired-image follow-up
