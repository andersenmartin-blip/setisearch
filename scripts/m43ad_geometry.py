"""Run the prospectively frozen M43AD geometry-aware development comparison."""
import argparse
import gzip
import hashlib
...
from seti_repeater.attribution_m43ab import centered_signatures, reclassify, apply_controls as centered_controls
from seti_repeater.attribution_m43ad import attributed_signatures, apply_controls, POLICIES

ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT/'results_m43ad_geometry'
CONFIG = ROOT/'config/m43ad_geometry.json'
...
def policy_decisions(audits):
    return {p:[dict(record_id=m['record_id'],
                   ...
                   rejections=m.get('m43ad_rejections', m.get('m43ab_rejections', m.get('m43z_rejections', []))))
               for m in a['members']] for p, a in audits.items()}


def costs(endpoints, baseline_counts, baseline_evidence_complete):
    out = {}
    for panel in ('historical', 'fresh'):
        refs = {r['name']:r for r in endpoints if r['panel'] == panel and r['policy'] == 'neighbor9'}
        prior = {r['name']:r for r in endpoints if r['panel'] == panel and r['policy'] == 'centered_receiver_off_match_aggregate'}
        for policy in POLICIES:
            ...
            prior_losses = [r['name'] for r in rows if r['signal_present'] and prior[r['name']]['recovered'] and not r['recovered']]
            cond['preserve_centered_combination_signals'] = not prior_losses
            cond['complete_additional_evidence'] = (all(r['additional_evidence_complete'] for r in rows)
                and (policy != POLICIES[-1] or baseline_evidence_complete))
            out[f'{panel}:{policy}'] = dict(signal_gains=gains, signal_losses=losses, centered_combination_losses=prior_losses,
...
def run(runtime, freeze):
    ...
    assert subprocess.check_output(['git', 'show', freeze+':config/m43ad_geometry.json'], cwd=ROOT) == CONFIG.read_bytes()
    assert platform.python_version() == cfg['python_version'] and np.__version__ == cfg['numpy_version']
    assert all(importlib.metadata.version(k) == v for k,v in cfg['dependencies'].items())
    ...
    history = {}
    for name, spec in cfg['historical_sources'].items():
        path = ROOT/spec['file']; assert sha(path) == spec['file_sha256']
        ...
        assert r['result_sha256'] == spec['record_sha256']
        history[name] = r
    ...
    on_factors = [core.factor_table_for_scan(table, basis, f'epoch{e+1}_on') for e in range(3)]
    off_factors = [core.factor_table_for_scan(table, basis, f'epoch{e+1}_off') for e in range(3)]
    endpoints = []; inventory = []; direct_total = 0; baseline_counts = None; baseline_complete = None
    specs = [dict(name='baseline', panel='baseline', components=[], original_case_index=None, source_name='baseline')]+cfg['cases']
    for index, case in enumerate(specs):
        ...
        if path.exists():
            ...
            new_audits, evidence = centered_controls(new_audit, store, grid); audits.update(new_audits)
            # Exact replay of all seven old endpoints in the selected regression
            # panel. No historical result or denominator is overwritten.
            if case['panel'] in ('historical', 'baseline'):
                h = history[case['source_name']]
                assert audits['neighbor9'] == h['reference_audit'], case['name']
                assert old_evidence == h['old_confirmation_evidence'], case['name']
                assert evidence == h['new_confirmation_evidence'], case['name']
                assert sigs == h['centered_signatures'] and result['receiver_signatures'] == h['original_signatures']
                assert policy_decisions(audits) == h['policy_decisions'], case['name']
            selected, geo_receipt, geometry = attributed_signatures(result['retained']['on'],
                result['receiver_signatures'], sigs, on_factors, grid)
            geo_result = reclassify(result, selected, geo_receipt, grid, bank, table, basis,
                metadata['scans'], WINDOW, cfg['maximum_records'])
            geo_result['schema'] = 'm43ad-geometry-attributed-receiver-v1'
            geo_result.pop('result_sha256'); geo_result['result_sha256'] = detector.digest(geo_result)
            geo_audit = compact(geo_result, overlay.overlay_receipt)
            profile_checks = []
            def check_profile(kind, e, t, w, support_index, score):
                src = receiver.cache(f'epoch{e+1}_{kind}', 1).source
                direct = raw_window_score(src, overlay.joint_indices[kind,e][t,:,support_index],
                    parts.get((kind,e), []), w)
                assert np.float32(direct['score']).view('<u4') == np.float32(score).view('<u4'), (case['name'],kind,e,t,w,support_index)
                profile_checks.append(dict(kind=kind, epoch=e, template=t, width=w,
                    support_index=support_index, score=score, exact=True))
            geo_audits, geo_evidence = apply_controls(geo_audit, store, grid, on_factors, off_factors, check_profile)
            audits.update(geo_audits)
            coverage_complete = all(e['aligned_complete'] for e in geo_evidence)
            rows = []
            if case['panel'] != 'baseline':
                for p, a in audits.items():
                    ...
                    rows.append(dict(name=case['name'], panel=case['panel'], case_index=case['case_index'],
                        ...
                        additional_evidence_complete=coverage_complete if p == POLICIES[-1] else True))
            ...
            rec = dict(case=case, config_sha256=sha(CONFIG), freeze_commit=freeze,
                ...
                geometry_alias=geo_result['receiver_alias'], geometry_confirmation_evidence=geo_evidence,
                profile_direct_checks=profile_checks, additional_evidence_complete=coverage_complete,
        ...
        direct_total += len(rec['direct_checks'])+len(rec['profile_direct_checks']); endpoints.extend(rec['endpoints'])
        if case['panel'] == 'baseline':
            baseline_counts = rec['final_counts']; baseline_complete = rec['additional_evidence_complete']
    ...
    write_sealed(OUT/'result.json', dict(milestone='M43AD', complete=True, freeze_commit=freeze,
        base_executions_including_baseline=len(specs), paired_endpoints_including_baseline=len(specs)*len(cfg['policies']),
        historical_inputs=37, fresh_inputs=112, separate_baseline=1, endpoints=endpoints, inventory=inventory,
        direct_native_checks=direct_total, baseline_counts=baseline_counts, summary=summary,
        baseline_additional_evidence_complete=baseline_complete,
        comparisons=costs(endpoints, baseline_counts, baseline_complete), distinct_native_payloads=len({r['native_payload_identity'] for r in inventory}),
    ...
    print('M43AD COMPLETE', flush=True)
