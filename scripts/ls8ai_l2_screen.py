#!/usr/bin/env python3
"""Transfer pinned LS8K acquisition/scoring functions to the frozen EC13080-1508 pair."""
import argparse
import hashlib
import json
...
META = ROOT / 'results_ls8ai_l2_metadata'
...
def main():
    ...
    assert not OUT.exists(), 'refuse LS8AI output overwrite'
    cfg = json.loads((ROOT / 'config/ls8ai_l2_scope.json').read_text())
    ...
    summary = {'stage': 'LS8AI_EC130801508_TWO_VISIT_L2_SCREEN', 'status': 'COMPLETE_UNAUDITED',
        'evaluation_freeze_commit': args.freeze, 'target': 'EC13080-1508', 'selected_keys': KEYS,
        ...
        'interpretation': 'Independent rank-11 two-visit L2 screen; signed endpoints are not Gaussian significances.'}
