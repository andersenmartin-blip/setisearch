#!/usr/bin/env python3
"""Report every fixed LS8AG image outcome without altering the diagnostics."""
import json
import math
...
def main():
    ...
    cfg=json.loads((ROOT/'config/ls8ag_images.json').read_text())
    ...
    if unresolved:
        ...
        next_action=('Close this PG 1207-033 follow-up under its fixed descriptive label without widening. '
                     'The next independent target is rank 11 of the unchanged reconciled LS8J ledger, EC13080-1508; '
                     'freeze its exact pair and header preflight before science values.')
    count=audit['comparisons']
    text=f'''# LS8AG — PG 1207-033 positive-event paired-image follow-up

Completed 22 September 2026. **COMPLETE_AUDITED**, independent audit **PASS**.

The separately frozen diagnostic retains the complete LS8AF signed representative
...
The positive representative is TG000901 row 60, one 60-second exposure,
with NEXP=1. Its original L2 score is +141.1054591837304, not a Gaussian
...
LS8AF's L2 audit remains PASS. Historical LS8B's original FAIL and later
