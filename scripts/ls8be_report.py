#!/usr/bin/env python3
"""Report every fixed LS8BE image outcome without altering the diagnostics."""
import json
import math
...
def main():
    ...
    cfg=json.loads((ROOT/'config/ls8be_images.json').read_text())
    ...
    if unresolved:
        ...
        next_action=('Close this GJ 494 follow-up under its fixed descriptive label without widening. '
                     'The next independent target is rank 24 of the unchanged reconciled LS8J ledger, 2MASS J11474440+0048164; '
                     'freeze its exact pair and header preflight before science values.')
    l2meta=json.loads((ROOT/'results_ls8bd_l2_metadata/summary.json').read_text())
    ...
    for context in cfg['contexts']:
        ...
        original=json.loads((ROOT/'results_ls8bd_l2_screen'/key/'clusters.json').read_text())
    ...
    text=f'''# LS8BE — GJ 494 complete signed-event paired-image follow-up
