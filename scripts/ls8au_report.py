#!/usr/bin/env python3
"""Readable report and figure from retained LS8AU tables and audit, no network."""
import gzip
import hashlib
import json
...
OUT = ROOT/'results_ls8au_l2_screen'


def main():
    s = json.loads((OUT/'summary.json').read_text())
    a = json.loads((OUT/'audit.json').read_text())
    meta = json.loads((ROOT/'results_ls8au_l2_metadata/summary.json').read_text())
    ...
    for column, visit in enumerate(s['visits']):
        ...
        axes[0, column].set_title(visit['file_key'].split('_')[2]+' · LS IV +09 2')
    ...
    fig.suptitle('LS8AU · two prospectively selected CHEOPS visits', fontsize=15, fontweight='bold')
    fig.savefig(OUT/'lsiv092_screen.png', dpi=180)
    plt.close(fig)
    rows = []
    for v in s['visits']:
    ...
    if representatives:
        ...
        followup='Close this two-visit LS IV +09 2 screen as a descriptive null. The next independent cohort is rank 18 of the unchanged reconciled LS8J ledger, GJ 9404. Freeze its exact pair and header preflight before accessing that cohort. Do not widen or retune the closed LS IV +09 2 pair.'
        reps=''
    if a['status']!='PASS':
    ...
    text=f'''# LS8AU — LS IV +09 2 two-visit DEFAULT-L2 screen
