#!/usr/bin/env python3
"""Readable report and figure from retained LS8BA tables and audit, no network."""
import gzip
import hashlib
import json
...
OUT = ROOT/'results_ls8ba_l2_screen'


def main():
    s = json.loads((OUT/'summary.json').read_text())
    a = json.loads((OUT/'audit.json').read_text())
    meta = json.loads((ROOT/'results_ls8ba_l2_metadata/summary.json').read_text())
    assert a['status']=='PASS', 'preserve any audit failure before report/promotion'
    t = s['totals']
    duration_text = '; '.join(v['file_key'] + ': ' + '/'.join(f"{d['duration_seconds']:.8g}" for d in v['duration_results']) + ' seconds' for v in s['visits'])
    ...
    for column, visit in enumerate(s['visits']):
        ...
        axes[0, column].set_title(visit['file_key'].split('_')[2]+' · 2MASS J11285624+1010395')
    ...
    fig.suptitle('LS8BA · two prospectively selected CHEOPS visits', fontsize=15, fontweight='bold')
    fig.savefig(OUT/'2massj11285624_screen.png', dpi=180)
    plt.close(fig)
    rows = []
    for v in s['visits']:
    ...
    if representatives:
        ...
        followup='Close this two-visit 2MASS J11285624+1010395 screen as a descriptive null. The next independent cohort is rank 22 of the unchanged reconciled LS8J ledger, GJ 422. Freeze its exact pair and header preflight before accessing that cohort. Do not widen or retune the closed 2MASS J11285624+1010395 pair.'
        reps=''
    if a['status']!='PASS':
    ...
    text=f'''# LS8BA — 2MASS J11285624+1010395 two-visit DEFAULT-L2 screen
...
![Retained light curves and all eligible signed scores](2massj11285624_screen.png)
...
2MASS J11285624+1010395 was rank 21 of the metadata-only first-1,000-public-row CHEOPS census:
...
are one/two/three rows ({duration_text}), with 12 sideband rows on each side, two-row guards
