#!/usr/bin/env python3
"""Readable report and figure from retained LS8V tables and audit, no network."""
import gzip
import hashlib
import json
...
OUT = ROOT/'results_ls8v_l2_screen'


def main():
    s = json.loads((OUT/'summary.json').read_text())
    a = json.loads((OUT/'audit.json').read_text())
    meta = json.loads((ROOT/'results_ls8v_l2_metadata/summary.json').read_text())
    ...
    for column, visit in enumerate(s['visits']):
        ...
        axes[0, column].set_title(visit['file_key'].split('_')[2]+' · EC 12578-2107')
    ...
    fig.suptitle('LS8V · two prospectively selected CHEOPS visits', fontsize=15, fontweight='bold')
    fig.savefig(OUT/'ec125782107_screen.png', dpi=180)
    plt.close(fig)
    rows = []
    for v in s['visits']:
    ...
    if representatives:
        ...
        followup='Close this two-visit EC 12578-2107 screen as a descriptive null. The next independent cohort is rank 6 of the unchanged reconciled LS8J ledger, GJ 436. Freeze its exact pair and header preflight before accessing that cohort. Do not widen or retune the closed EC 12578-2107 pair.'
        reps=''
    if a['status']!='PASS':
    ...
    text=f'''# LS8V — EC 12578-2107 two-visit DEFAULT-L2 screen

Completed 21 September 2026. Result: **{s['status']}**.
...
![Retained light curves and all eligible signed scores](ec125782107_screen.png)
...
EC 12578-2107 was rank 5 of the metadata-only first-1,000-public-row CHEOPS census:
...
Image bytes are zero; no alternative aperture, raw imagette or later EC 12578-2107
visit was opened.
