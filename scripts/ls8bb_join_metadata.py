...
META = ROOT / 'results_ls8bb_metadata'


def main():
    reps = json.loads((ROOT/'config/ls8bb_representatives.json').read_text())['representatives']
    expected=[]
    for key in ['CH_PR100018_TG010801_V0300','CH_PR100018_TG010802_V0300']:
    ...
    assert reps==expected and len(reps)==2, 'all signed representatives must be retained'
    sources, tables, l2 = {}, {}, {}
    for key in dict.fromkeys(r[0] for r in reps):
        h = fits.Header.fromstring((ROOT/'results_ls8ba_l2_metadata'/key/'lightcurve_header.bin').read_bytes().decode(), sep='')
        l2[key] = np.frombuffer((ROOT/'results_ls8ba_l2_screen'/key/'lightcurve_table.bin').read_bytes(), dtype=dtype(h))
    ...
    manifest={'stage':'LS8BB','status':'METADATA_JOINED_IMAGES_CLOSED','sources':sources,'contexts':contexts,
    ...
    (ROOT/'config/ls8bb_images.json').write_text(json.dumps(manifest,indent=2)+'\n')
