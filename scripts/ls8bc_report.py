#!/usr/bin/env python3
"""Readable report and figure from retained LS8BC tables and audit, no network."""
import gzip
import hashlib
import json
...
OUT = ROOT/'results_ls8bc_l2_screen'


def main():
    s = json.loads((OUT/'summary.json').read_text())
    a = json.loads((OUT/'audit.json').read_text())
    meta = json.loads((ROOT/'results_ls8bc_l2_metadata/summary.json').read_text())
    ...
    for column, visit in enumerate(s['visits']):
        ...
        axes[0, column].set_title(visit['file_key'].split('_')[2]+' · GJ 422')
    ...
    fig.suptitle('LS8BC · two prospectively selected CHEOPS visits', fontsize=15, fontweight='bold')
    fig.savefig(OUT/'gj422_screen.png', dpi=180)
    plt.close(fig)
    rows = []
    for v in s['visits']:
    ...
    if representatives:
        ...
        followup='Close this two-visit GJ 422 screen as a descriptive null. The next independent cohort is rank 23 of the unchanged reconciled LS8J ledger, GJ 494. Freeze its exact pair and header preflight before accessing that cohort. Do not widen or retune the closed GJ 422 pair.'
        reps=''
    if a['status']!='PASS':
    ...
    text=f'''# LS8BC — GJ 422 two-visit DEFAULT-L2 screen
