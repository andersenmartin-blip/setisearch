#!/usr/bin/env python3
"""Transfer pinned LS8K acquisition/scoring functions to the frozen PG1303-114 pair."""
import argparse
import hashlib
import json
...
META = ROOT / 'results_ls8ac_l2_metadata'
...
def main():
    ...
    assert not OUT.exists(), 'refuse LS8AC output overwrite'
    cfg = json.loads((ROOT / 'config/ls8ac_l2_scope.json').read_text())
    ...
    summary = {'stage': 'LS8AC_PG1303114_TWO_VISIT_L2_SCREEN', 'status': 'COMPLETE_UNAUDITED',
        'evaluation_freeze_commit': args.freeze, 'target': 'PG1303-114', 'selected_keys': KEYS,
        ...
        'interpretation': 'Independent rank-9 two-visit L2 screen; signed endpoints are not Gaussian significances.'}
