#!/usr/bin/env python3
"""Prove extraction coverage for every scan, window, and frozen template.
...
from seti_repeater.search import make_rest_grid, make_templates
...
def check_config(config: dict) -> dict:
    ...
    for window in config["windows"]:
        ...
        for scan in config["scans"]:
            ...
            for template_index, (scale, phase) in enumerate(templates):
                ...
                lower_headroom = int(indices.min() - margin)
                upper_headroom = int(nfreq - margin - 1 - indices.max())
                ...
                template_records.append({
                    ...
                    "passed": bool(passed),
                    "dedoppler_margin_channels": margin,
    ...
    return {
        "purpose": "Metadata-only full-bank extraction coverage proof",
