#!/usr/bin/env python3
"""Readable report and figure from retained LS8K tables and audit, no network."""
import gzip
import hashlib
import json
...
OUT = ROOT/'results_ls8k_l2_screen'


def main():
    s = json.loads((OUT/'summary.json').read_text())
    a = json.loads((OUT/'audit.json').read_text())
    meta = json.loads((ROOT/'results_ls8k_l2_metadata/summary.json').read_text())
    t = s['totals']
    ...
    for column, visit in enumerate(s['visits']):
        ...
        axes[0, column].set_title(visit['file_key'].split('_')[2]+' · WASP-189')
    ...
    fig.suptitle('LS8K · two prospectively selected CHEOPS visits', fontsize=15, fontweight='bold')
    fig.savefig(OUT/'wasp189_screen.png', dpi=180)
    plt.close(fig)
    rows = []
    for v in s['visits']:
    ...
    if representatives:
        ...
        followup='Close this two-visit WASP-189 screen as a descriptive null. The next independent cohort is rank 2 of the unchanged reconciled LS8J ledger, GJ 1132. Freeze its exact pair and header preflight before accessing that cohort. Do not add the later WASP-189 visits.'
        reps=''
    if a['status']!='PASS':
    ...
    text=f'''# LS8K — WASP-189 two-visit DEFAULT-L2 screen
