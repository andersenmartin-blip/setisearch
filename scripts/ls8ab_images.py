#!/usr/bin/env python3
"""Frozen LS8AB range acquisition and paired-image analysis, with offline reuse."""
import argparse
...
def main():
    ...
    scope=json.loads((ROOT/'config/ls8ab_payload_scope.json').read_text())
    for name,digest in scope['input_pins'].items():
        assert sha((ROOT/name).read_bytes())==digest,name
    cfg=json.loads((ROOT/'config/ls8ab_images.json').read_text())
    ...
    assert [(c['id'],c['start'],c['duration'],c['lo'],c['hi']) for c in cfg['contexts']]==[
        ('TG007801_P0',62,1,48,77)]
    assert json.loads((ROOT/'results_ls8ab_metadata/audit.json').read_text())['status']=='PASS'
    for line in (ROOT/'results_ls8ab_metadata/SHA256SUMS').read_text().splitlines():
        digest,relative=line.split('  ',1)
        assert sha((ROOT/'results_ls8ab_metadata'/relative).read_bytes())==digest,relative
    ...
    for c in cfg['contexts']:
        ...
        if key not in tables:
            header=fits.Header.fromstring((ROOT/'results_ls8aa_l2_metadata'/key/'lightcurve_header.bin').read_bytes().decode(),sep='')
            tables[key]=np.frombuffer((ROOT/'results_ls8aa_l2_screen'/key/'lightcurve_table.bin').read_bytes(),dtype=dtype(header))
