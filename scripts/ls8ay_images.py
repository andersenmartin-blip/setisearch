#!/usr/bin/env python3
"""Frozen LS8AY range acquisition and paired-image analysis, with offline reuse."""
import argparse
...
def main():
    ...
    scope=json.loads((ROOT/'config/ls8ay_payload_scope.json').read_text())
    ...
    assert cfg['join_rows']==522 and cfg['image_science_bytes']==167040000 and cfg['smearing_bytes']==417600
    assert [(c['id'],c['start'],c['duration'],c['lo'],c['hi']) for c in cfg['contexts']]==[('TG023501_P0', 1816, 1, 1802, 1831), ('TG023501_P1', 1849, 1, 1835, 1864), ('TG023501_P2', 2573, 1, 2559, 2588), ('TG023501_P3', 2770, 1, 2756, 2785), ('TG023501_P4', 3082, 1, 3068, 3097), ('TG023501_P5', 3323, 1, 3309, 3338), ('TG023501_P6', 3408, 1, 3394, 3423), ('TG023501_N0', 802, 1, 788, 817), ('TG007801_P0', 17, 1, 3, 32)]
    assert json.loads((ROOT/'results_ls8ay_metadata/audit.json').read_text())['status']=='PASS'
    for line in (ROOT/'results_ls8ay_metadata/SHA256SUMS').read_text().splitlines():
        digest,relative=line.split('  ',1)
        assert sha((ROOT/'results_ls8ay_metadata'/relative).read_bytes())==digest,relative
    ...
    for c in cfg['contexts']:
        ...
        if key not in tables:
            ...
            tables[key]=np.frombuffer((ROOT/'results_ls8ax_l2_screen'/key/'lightcurve_table.bin').read_bytes(),dtype=dtype(header))
