#!/usr/bin/env python3
"""Fixed post-hoc morphology checks for the two Milestone 21 review cases.

The implementation reuses the already published Milestone 11 local-cutout
measurements without modifying that historical script. It evaluates every
Milestone 21 arithmetic-family cases together so the fixed cross-candidate
receiver-frame-alias check is applied across both.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("config/hd154345b_heldout_m21.json"),
    )
    parser.add_argument(
        "--summary",
        type=Path,
        default=Path("results_m21/search_summary.json"),
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
        default=Path("data_m21_candidate_investigation"),
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results_m21_candidate_investigation"),
    )
    parser.add_argument("--extract", action="store_true")
    parser.add_argument("--workers", type=int, default=12)
    parser.add_argument("--date-utc", default="2026-08-23")
    ...
    archive = {
        ...
        "interpretation": (
            "The frozen Milestone 21 header-only screen found no second "
            "qualifying HD 154345 cadence. Any unresolved case still requires a "
            "later independent public observation."
        ),
    }
    result = {
        "analysis_label": "Milestone 21 post-hoc candidate investigation",
        "detector_status": (
