#!/usr/bin/env python3
"""Readable report and figure from retained LS8BD tables and audit, no network."""
import gzip
import hashlib
import json
...
OUT = ROOT/'results_ls8bd_l2_screen'


def main():
    s = json.loads((OUT/'summary.json').read_text())
    a = json.loads((OUT/'audit.json').read_text())
    meta = json.loads((ROOT/'results_ls8bd_l2_metadata/summary.json').read_text())
    ...
    for column, visit in enumerate(s['visits']):
        ...
        axes[0, column].set_title(visit['file_key'].split('_')[2]+' · GJ 494')
    ...
    fig.suptitle('LS8BD · two prospectively selected CHEOPS visits', fontsize=15, fontweight='bold')
    fig.savefig(OUT/'gj494_screen.png', dpi=180)
    plt.close(fig)
    rows = []
    for v in s['visits']:
    ...
    if representatives:
        ...
        followup='Close this two-visit GJ 494 screen as a descriptive null. The next independent cohort is rank 24 of the unchanged reconciled LS8J ledger, 2MASS J11474440+0048164. Freeze its exact pair and header preflight before accessing that cohort. Do not widen or retune the closed GJ 494 pair.'
        reps=''
    if a['status']!='PASS':
    ...
    text=f'''# LS8BD — GJ 494 two-visit DEFAULT-L2 screen

Completed 24 September 2026. Result: **{s['status']}**.
...
![Retained light curves and all eligible signed scores](gj494_screen.png)
...
GJ 494 was rank 23 of the metadata-only first-1,000-public-row CHEOPS census:
