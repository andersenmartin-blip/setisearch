...
META = ROOT / 'results_ls8aj_metadata'


def main():
    reps = json.loads((ROOT/'config/ls8aj_representatives.json').read_text())['representatives']
    expected=[]
    for key in ['CH_PR100002_TG005201_V0300','CH_PR100002_TG005202_V0300']:
        clusters=json.loads((ROOT/'results_ls8ai_l2_screen'/key/'clusters.json').read_text())
        for sign in ('positive','negative'):
            for c in clusters[sign]:
                expected.append([key,sign,c['cluster_id'],c['representative']['start'],c['representative']['duration']])
    assert reps==expected and len(reps)==1, 'all signed representatives must be retained'
    sources, tables, l2 = {}, {}, {}
    for key in dict.fromkeys(r[0] for r in reps):
        h = fits.Header.fromstring((ROOT/'results_ls8ai_l2_metadata'/key/'lightcurve_header.bin').read_bytes().decode(), sep='')
        l2[key] = np.frombuffer((ROOT/'results_ls8ai_l2_screen'/key/'lightcurve_table.bin').read_bytes(), dtype=dtype(h))
    ...
    manifest={'stage':'LS8AJ','status':'METADATA_JOINED_IMAGES_CLOSED','sources':sources,'contexts':contexts,
    ...
    (ROOT/'config/ls8aj_images.json').write_text(json.dumps(manifest,indent=2)+'\n')
