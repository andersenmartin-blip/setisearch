#!/usr/bin/env python3
"""Transfer pinned LS8K acquisition/scoring functions to the frozen EC 12578-2107 pair."""
import argparse
import hashlib
import json
...
META = ROOT / 'results_ls8v_l2_metadata'
...
def main():
    ...
    assert not OUT.exists(), 'refuse LS8V output overwrite'
    cfg = json.loads((ROOT / 'config/ls8v_l2_scope.json').read_text())
    ...
    summary = {'stage': 'LS8V_EC125782107_TWO_VISIT_L2_SCREEN', 'status': 'COMPLETE_UNAUDITED',
        'evaluation_freeze_commit': args.freeze, 'target': 'EC 12578-2107', 'selected_keys': KEYS,
        ...
        'interpretation': 'Independent rank-5 two-visit L2 screen; signed endpoints are not Gaussian significances.'}
