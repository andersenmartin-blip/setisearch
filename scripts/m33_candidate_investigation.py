#!/usr/bin/env python3
"""Fixed post-hoc morphology checks for the Milestone 33 review case.

The implementation reuses the published Milestone 11 local-cutout
measurements without modifying that historical script. It evaluates the one
Milestone 33 arithmetic-family case under the already established physical
ON/control morphology protocol.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("config/hd3651b_heldout_m33.json"),
    )
    parser.add_argument(
        "--summary",
        type=Path,
        default=Path("results_m33/search_summary.json"),
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
        default=Path("data_m33_candidate_investigation"),
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results_m33_candidate_investigation"),
    )
    parser.add_argument("--extract", action="store_true")
    parser.add_argument("--workers", type=int, default=12)
    parser.add_argument("--date-utc", default="2026-08-26")
    ...
    archive = {
        ...
        "interpretation": (
            "The Milestone 33 header-only screen found no second qualifying "
            "HD 3651 cadence. Any unresolved case therefore requires a new "
