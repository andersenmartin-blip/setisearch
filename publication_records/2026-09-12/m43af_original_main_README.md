...
**Current work: M43AF is complete; the frozen joint response rule does not qualify.**
All **502 prescribed records** and the independent whole-study audit are complete
under the unchanged scientific freeze
[75b271b](https://github.com/andersenmartin-blip/setisearch/commit/75b271b4b92819783692375687586d6df4f40c57).
The full record set contains **54,140 complete profiles**: 44,746 newly acquired
and 9,394 reused exact profiles, with zero incomplete profiles or undefined
member measurements. The 261 historical records preserve the original detector
endpoints and provide diagnostic response measurements.

The independent auditor checks all **1,156 frozen training-grid points** and
...
![Frozen training grid and unachieved qualification target](https://raw.githubusercontent.com/andersenmartin-blip/setisearch/m43-support-qualification/results_m43af_response/figures/training_grid_tradeoff.svg)

These are descriptive costs within the fixed grid. No new rule is selected or
adopted. The **112 validation inputs and 128 held-out native nulls remain unopened**.
The three inherited weak-epoch loss cases now have complete ON/OFF response
coordinates; their original failures remain unchanged.

All 940 pinned scientific files remain unchanged. The complete lossless archive
reconstructs **508 original files from 69 parts**, including all 502 records,
the full grid, immutable decision, publication receipt and audits. All 61 focused
tests pass. Runtime recovery reproduces all six sources, 96 arrays and 7,503,600
original score values exactly; the earlier training archive remains intact.

All inputs share one observing sequence. The native nulls and baseline have no
eligible members and provide no conditional profile-tail observations. No physical
false-alarm probability, additional sky coverage or astronomical candidate is claimed.
A revised rule requires a new prospective study with fresh evaluation inputs;
general adoption also requires an independent observing sequence.

[Complete results, reference outcomes and evidence](https://github.com/andersenmartin-blip/setisearch/blob/m43-support-qualification/MILESTONE_43AF_RESPONSE_STUDY_RESULT.md).
[Current continuation](https://github.com/andersenmartin-blip/setisearch/blob/m43-support-qualification/M43AF_CURRENT_CONTINUATION.md).
[Complete lossless archive](https://github.com/andersenmartin-blip/setisearch/tree/m43-support-qualification/results_m43af_response/complete_archive).
...
**Previous development result — M43AD completed; all three new policies fail qualification.**
...
widths; all 1,728 batches and 54,432 full vectors pass. All 579,705,984 valid
native-filter values also match directly, including their row and payload hashes.
Together with M43J/L, numerical integrated-score coverage now spans all six
sources at all eight widths: 61,055,802,864 compared cells in one window across
three ON/OFF pairs. These are repeated scans in one observing sequence and
correlated numerical comparisons, not independent trials or candidate counts.
All 81 M43-family tests and the 42-file manifest pass. The protocol was public
before evaluation; 28 completed wider-width checkpoints were published while
the final width-one checks continued. No new downloads or run failures occurred.
All 32 completed checkpoints, the final audit and results are published. Next:
real multi-epoch stack/detection validation, then separately frozen null and
injection/recovery calibration. No candidates were selected in this qualification.
See [the M43N results, numerical scope and reproduction](https://github.com/andersenmartin-blip/setisearch/blob/m43-support-qualification/MILESTONE_43N_EPOCH_SCORES_RESULT.md).

**Previous project update — M43M completes the three-pair source inventory.**
...
See [the LS4K report and policy comparison](https://github.com/andersenmartin-blip/setisearch/blob/ls4k-off-policy-counterexamples/LS4K_OFF_POLICY_RESULT.md).
